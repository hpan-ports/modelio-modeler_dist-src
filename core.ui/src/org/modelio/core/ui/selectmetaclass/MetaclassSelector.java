...
package org.modelio.core.ui.selectmetaclass;
 ...
 * 
 * 
 * Note: SelectMetaclass wraps a SWT Text because inheriting from Text is not possible. Therefore the getTextControl() method is
...
public class MetaclassSelector {
    ...
    public MClass getSelected() {
        return this.metamodel.getMClass(MetaclassSelector.this.text.getText());
    }
    ...
    private Text createControl(Composite parent, int style) {
        ...
        ContentProposalAdapter adapter = new ContentProposalAdapter(wrappedText, new TextContentAdapter(),
                new SelectMetaclassContentProposalProvider(this), null, null);
    ...
    private void showMetaclassValidity() {
        final MClass mClass = this.metamodel.getMClass(MetaclassSelector.this.text.getText());
        if (mClass != null) {
            MetaclassSelector.this.text
            .setForeground(MetaclassSelector.this.text.getDisplay().getSystemColor(SWT.COLOR_DARK_GREEN));
