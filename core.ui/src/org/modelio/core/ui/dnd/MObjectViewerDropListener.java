...
public class MObjectViewerDropListener extends ViewerDropAdapter {
    @objid ("b2ad6d1e-f420-4112-9b25-e8afda780e39")
    ...
    public boolean validateDrop(Object target, int operation, TransferData transferType) {
        ...
        if (refs != null) {
            ...
            for (int i = 0; i < refs.length ; i++) {
                dropedElements.add(this.coreSession.getModel().findByRef(refs[i], IModel.NODELETED));
            }
    ...
    private boolean isValidDrop(MObject targetElement, List<MObject> elementsToDrop, boolean isMove) {
        ...
        for (final MObject element : elementsToDrop) {
            ...
            if (selectionAreFreeCmsNode && !isFreeCmsNode) selectionAreFreeCmsNode = false;
    ...
    private boolean performDrop(MObject targetElement, List<MObject> elementsToDrop, boolean isMove) {
        ...
        // Perform the action
        ITransactionSupport transactionManager = this.coreSession.getTransactionSupport();
