 ...
 * <li>a logo image displayed on the right side, set with
 * {@link #setLogoImage(Image)}
 * <li>a message, set with {@link #setMessage(String)}.
 * </ul>
 * <li>a user defined content area defined with
 * {@link #createContentArea(Composite)}.
 * <li>a button area with user defined buttons defined with
 * {@link #addButtonsInButtonBar(Composite)}.
 * <li>An optional message area that display on top of the header area, showing
 * a warning or error small icon and a message.<br>
 * See {@link #setWarningMessage(String)} and {@link #setErrorMessage(String)}.
 * </ul>
 */
@objid ("0049a19c-3b99-1fcf-9f44-001ec947cd2a")
public abstract class ModelioDialog extends TrayDialog implements IModelioDialog {
    ...
    protected Control createContents(Composite parent) {
        ...
        ImageDescriptor imageDescriptor = AbstractUIPlugin.imageDescriptorFromPlugin("org.modelio.core.ui", "icons/headerleft.png");
    ...
    private void setMessageAreaVisible(boolean visible) {
        ...
        if (!this.messageArea.isVisible() && this.messageArea.getBounds().y != bottom)
            this.messageArea.setBounds(this.messageImageLabel != null ? this.messageImageLabel.getBounds().x : msgLabelBounds.x,
                    bottom, this.messageImageLabel != null ? msgLabelBounds.width + this.messageImageLabel.getBounds().width
                            : msgLabelBounds.width, this.messageArea.computeSize(-1, -1).y);
