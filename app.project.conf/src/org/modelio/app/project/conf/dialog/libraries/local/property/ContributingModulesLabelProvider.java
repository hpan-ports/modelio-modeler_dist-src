...
public class ContributingModulesLabelProvider extends StyledCellLabelProvider {
    ...
    private List<GModule> modules;

    @objid ("9fdb810d-f710-45a1-8e5c-cd504a81466d")
    public ContributingModulesLabelProvider(List<GModule> modules) {
        super();
    ...
    private StyledString getStyledText(VersionedItem<?> item) {
        ...
        String text;
        if (version == null) {
            text = item.getName() + " (" + AppProjectConf.I18N.getString("PackageRamcDialog.NoVersion") + ")";
        } else {
