...
public class RuntimePattern implements Comparable<RuntimePattern> {
    ...
    public RuntimePattern(Package modelPattern) {
        ...
        String projectPath = Modelio.getInstance().getContext().getProjectSpacePath().toString();
        this.patternPath = Paths.get(projectPath).resolve("patterns").resolve(this.jaxbPattern.getName() + "_" + this.jaxbPattern.getVersion() + ".umlt");
    }
    ...
    public RuntimePattern(Path patternPath) throws IOException, JAXBException {
    ...
    private void addConstantParameter(Pattern jaxbPattern, ModelElement element) {
        ...
        jaxbConstantParameter.setMetaclass(element.getMClass().getName());
    ...
    private void addElementParameter(Pattern jaxbPattern, ModelElement element) {
        ...
        jaxbElementParameter.setMetaclass(element.getMClass().getName());
    ...
    private void addModuleDependency(Pattern jaxbPattern, ModuleComponent element) {
        if (element.getName().equals(ProfileUtils.MODULE_NAME)) {
            return;
        }
    ...
    public void applyPattern(Map<String, Object> parameters, ICoreSession coreSession, MObject refElement) throws IOException {
        ...
        MMetamodel metamodel = session.getModel().getModelRoots().get(0).getMClass().getMetamodel();
        ...
        IPattern result = getExecutablePattern();
        if (result != null) {
    ...
    public IPattern getExecutablePattern() throws IOException {
        try (FileSystem fs = FileSystems.newFileSystem(this.patternPath, this.getClass().getClassLoader())) {
    ...
    public String getIconPath() {
        ...
        String s = "platform:/plugin/" + bundle.getSymbolicName() + "/" + "icons/Pattern16.png";
        URL url = null;
        try {
            url = new URL(s);
    ...
    public Image getImage() {
        ...
        return ImageRegistry.getImage("icons/Pattern16.png");
    }
    ...
    public boolean isRunnableOn(Collection<MObject> elements) {
        for (Object data : this.jaxbPattern.getCategoryAndExternalDependencyAndParameter()) {
            if (data instanceof RootParameter) {
                ...
                MMetamodel metamodel = Modelio.getInstance().getModelingSession().getModel().getModelRoots().get(0).getMClass()
                        .getMetamodel();
    ...
    private void addRootParameter(Pattern jaxbPattern, ModelElement elt) {
        ...
        jaxbRootParameter.setMetaclass(root.getMClass().getName());
