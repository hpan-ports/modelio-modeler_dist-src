...
public abstract class AbstractCreateHandler {
    ...
    /**
     * <p>
     * Abstract method to be implemented by subclasses. This method <strong>MUST</strong> return the main created element (or
     * <code>null</code> if nothing was created) so that it can be passed to the postCommitExecute method.
     * </p>
     * <p>
     * Note that although the modelling session is passed as parameter, a transaction has already been opened and will be committed
     * after this method returns.
     * </p>
     * @param modelingSession
     * the session this handler must be executed in.
     * @param selectedElement the element currently selected in the UML explorer view.
     * @return the created element (the main one if several are created at once) or <code>null</code> if nothing was created. This
     * returned element will be passed to the postCommitExecute method.
     * @throws org.eclipse.core.commands.ExecutionException if either the request metaclass, the requested dependency or the requested stereotype are invalid or cannot be
     * found.
     */
    ...
    /**
     * Subclasses should override this method to provide additional behaviours that should be only executed once the whole creation
     * transaction has been safely committed. For example, they can ask for the opening of a diagram editor.
