...
public class ClonePropertiesSelectionTool extends PanSelectionTool {
    ...
    protected boolean handleButtonDown(final int which) {
        ...
        ITransactionSupport session = ((IGmObject) target.getModel()).getDiagram().getModelManager().getModelingSession().getTransactionSupport();
        
        try (ITransaction transaction = session.createTransaction("Clone properties")) {
            ...
            // Save diagram
            DiagramPersistence.saveDiagram(((IGmObject) target.getModel()).getDiagram());
    ...
    private void cloneGraphicProperties(final GraphicalEditPart target) {
        ...
        final IStyle refStyle = refGm.getStyle();
        final IGmObject targetGm = (IGmObject) target.getModel();
        final IStyle targetStyle = targetGm.getStyle();
