...
public class RedrawConnectionTool extends AbstractConnectionCreationTool {
    ...
    /**
     * Constructs a new RedrawConnectionTool to redraw the given {@link GmLinkEditPart}
     * @param linkEditPart the edit part of the link to redraw.
     */
    @objid ("65c0663d-33f7-11e2-95fe-001ec947c8cc")
    public RedrawConnectionTool(final GmLinkEditPart linkEditPart) {
        super(new ModelioLinkCreationContext(((GmLink) linkEditPart.getModel()).getRelatedElement()));
        /*
    ...
    protected ConnectionRouterId getCurrentRoutingMode() {
        if (this.currentRoutingMode == null) {
            ...
            if (routerKey != null) {
                this.currentRoutingMode = this.originalGmLink.getStyle().getProperty(routerKey);
            }
        }
        if (this.currentRoutingMode == null) {
    ...
    protected void showTargetFeedback() {
        ...
        if (this.targetEditPart != null &&
            (CreateLinkConstants.REQ_CONNECTION_ADD_BENDPOINT.equals(getTargetRequest().getType()) || REQ_CONNECTION_END.equals(getTargetRequest().getType()))) {
            Object type = getTargetRequest().getType();
            getTargetRequest().setType(REQ_CONNECTION_END);
    ...
    public void deactivate() {
        ...
        super.deactivate();
        if (mustReload) {
            DiagramPersistence.loadDiagram(this.originalGmLink.getDiagram());
        }
    }
