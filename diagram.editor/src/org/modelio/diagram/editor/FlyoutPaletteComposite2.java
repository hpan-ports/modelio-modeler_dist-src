...
/**
 * The FlyoutPaletteComposite is used to show a flyout palette alongside another control. The flyout palette auto-hides (thus
 * maximizing space) when not in use, but can also be pinned open if so desired. It will only be visible when the PaletteView is
 * not.
 * 
...
public class FlyoutPaletteComposite2 extends Composite {
    ...
    /**
     * One of the two possible initial states of the flyout palette. This is the default one. When in this state, only the flyout
     * palette's sash is visible.
     */
    @objid ("6597de45-33f7-11e2-95fe-001ec947c8cc")
    public static final int STATE_COLLAPSED = 2;

    /**
     * One of the two possible initial states of the flyout palette. When in this state, the flyout palette is completely visible
     * and pinned open so that it doesn't disappear when the user wanders away from the flyout.
     */
    ...
    @objid ("659a409f-33f7-11e2-95fe-001ec947c8cc")
     int paletteState = STATE_HIDDEN;

    @objid ("659a40a0-33f7-11e2-95fe-001ec947c8cc")
     int paletteWidth = DEFAULT_PALETTE_SIZE;

    @objid ("659a40a1-33f7-11e2-95fe-001ec947c8cc")
     int minWidth = MIN_PALETTE_SIZE;
    ...
    public FlyoutPaletteComposite2(Composite parent, int style, PaletteViewerProvider pvProvider, FlyoutPreferences flyoutPreferences) {
        ...
        addListener(SWT.Resize, new Listener() {
            ...
            public void handleEvent(Event event) {
                ...
                /*
                 * @TODO:Pratik Sometimes, the editor is resized to 1,1 or 0,0 (depending on the platform) when the editor is closed
                 * or maximized. We have to ignore such resizes. See Bug# 62748
                 */
        ...
        this.listeners.addPropertyChangeListener(new PropertyChangeListener() {
            ...
            public void propertyChange(PropertyChangeEvent evt) {
                ...
                if (property.equals(PROPERTY_PALETTE_WIDTH))
                    ...
                    if (FlyoutPaletteComposite2.this.paletteState == STATE_COLLAPSED
                            || FlyoutPaletteComposite2.this.paletteState == STATE_PINNED_OPEN)
    ...
    public void layout(boolean changed) {
        ...
        /*
         * Fix for Bug# 65892 Laying out only when necessary helps reduce flicker on GTK in the case where the flyout palette is
         * being resized past its maximum size.
         */
    ...
    /**
     * If an external palette viewer is provided, palette state (that is captured in {@link PaletteViewer#saveState(IMemento)} --
     * active tool, drawer expansion state, drawer pin state, etc.) will be maintained when switching between the two viewers.
    ...
    void setPaletteWidth(int newSize) {
    ...
    /**
     * Sets the control along the side of which the palette is to be displayed. The given Control should be a child of this
    ...
    void setState(int newState) {
        /*
         * Fix for Bug# 69617 and Bug# 81248 FlyoutPreferences.getPaletteState() could return an invalid state if none is stored. In
         * that case, we use the default state: STATE_COLLAPSED.
         */
        ...
        case STATE_PINNED_OPEN:
            if (this.pViewer == null) {
                ...
                this.minWidth = Math.max(this.pViewer.getControl().computeSize(0, 0).x, MIN_PALETTE_SIZE);
            }
            break;
        case STATE_HIDDEN:
            if (this.pViewer == null)
        ...
        /*
         * Fix for Bug# 63901 When the flyout collapses, if the palette has focus, throw focus to the graphical control. That way,
         * hitting ESC will still deactivate the current tool and load the default one. Note that focus is being set on
         * RulerComposite and not GraphicalViewer's control. But this is okay since RulerComposite passes the focus on to its first
         * child, which is the graphical viewer's control.
         */
        if (this.paletteState == STATE_COLLAPSED && this.pViewer.getControl().isFocusControl())
    ...
    public interface FlyoutPreferences {
        /**
         * Should return {@link PositionConstants#EAST} or {@link PositionConstants#WEST}. Any other int will be ignored and the
        ...
        /**
         * When there is no saved state, this method can return any non-positive int (which will result in the palette using the
         * default state -- collapsed), or {@link FlyoutPaletteComposite#STATE_COLLAPSED}, or
        ...
        /**
         * When there is no saved width, this method can return any int (preferrably a non-positive int). Returning a non-positive
         * int will cause the palette to be sized to the default size, whereas returning a postive int will find the closest match
         * in the valid range (>= minimum and <= maximum)
        ...
        /**
         * This method is invoked when the flyout palette's dock location is changed. The provided dock location should be persisted
         * and returned in {@link #getDockLocation()}.
        ...
        /**
         * This method is invoked when the flyout palette's state is changed (the new state becomes the default). The provided state
         * should be persisted and returned in {@link #getPaletteState()}.
        ...
        /**
         * This method is invoked when the flyout palette is resized. The provided width should be persisted and returned in
    ...
    private class Sash extends Composite {
        ...
        public Point computeSize(int wHint, int hHint, boolean changed) {
            ...
            return new Point(SASH_BUTTON_WIDTH + 2, FlyoutPaletteComposite2.this.cachedTitleHeight);
        }
        ...
        public void layout(boolean changed) {
            ...
            Rectangle area = getClientArea();
            this.button.setBounds(area.x + 1, area.y + 1, SASH_BUTTON_WIDTH, FlyoutPaletteComposite2.this.cachedTitleHeight - 1);
        ...
        private class SashDragManager extends MouseAdapter implements MouseMoveListener {
            ...
            public void mouseDown(MouseEvent me) {
                if (me.button != 1)
                    return;
                this.mouseDown = true;
                this.correctState = isInState(STATE_EXPANDED | STATE_PINNED_OPEN);
                this.origX = me.x;
    ...
    private class TitleDragManager extends MouseAdapter implements Listener, MouseTrackListener {
                    ...
                    Display.getCurrent().syncExec(new Runnable() {
                        ...
                        public void run() {
                            ...
                            TitleDragManager.this.switchDock = isDescendantOf(FlyoutPaletteComposite2.this.graphicalControl, ctrl)
                                    && ((FlyoutPaletteComposite2.this.dock == PositionConstants.WEST && pt.x > TitleDragManager.this.threshold - 10) || (FlyoutPaletteComposite2.this.dock == PositionConstants.EAST && pt.x < TitleDragManager.this.threshold + 10));
        ...
        public void mouseHover(MouseEvent e) {
            /*
             * @TODO:Pratik Mouse hover events are received if the hover occurs just before you finish or cancel the drag. Open a
             * bugzilla about it?
             */
            if (isInState(STATE_COLLAPSED))
                setState(STATE_EXPANDED);
    ...
    private class PaletteComposite extends Composite {
        ...
        protected void updateState() {
            this.button.setVisible(isInState(STATE_PINNED_OPEN));
    ...
    private class TitleCanvas extends Canvas {
        ...
        public void setFont(Font font) {
            ...
            if (isVisible()) {
                /*
                 * If this canvas is in the sash, we want the FlyoutPaletteComposite to layout (which will cause the sash to be
                 * resized and laid out). However, if this canvas is in the paletteContainer, the paletteContainer's bounds won't
