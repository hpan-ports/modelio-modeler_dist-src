...
public class ContributorCategoryModel {
    ...
    private static final Image DEFAULT_DIAGRAM_IMAGE = getCategoryIcon("icons/diagram.png");

    @objid ("8d451f35-4718-4f48-8049-b2c79fe4b0d9")
    private static final Image DEFAULT_MATRIX_IMAGE = getCategoryIcon("icons/matrix.png");
    ...
    public List<Category> createCategories(Map<ContributorCategory, List<IWizardContributor>> contributorsMap) {
        ...
        for (Map.Entry<ContributorCategory, List<IWizardContributor>> entry : contributorsMap.entrySet()) {
            ...
            for (Category existingCategory : this.categories) {
                if (existingCategory.getLabel().equals(key.getLabel())) {
            ...
            // Build sub-categories for diagrams & matrices
            Category diagramSubCategory = new Category(CreationWizard.I18N.getString("Ui.CreationWizard.DiagramSubCategory"), DEFAULT_DIAGRAM_IMAGE);
            Category matrixSubCategory = new Category(CreationWizard.I18N.getString("Ui.CreationWizard.MatrixSubCategory"), DEFAULT_MATRIX_IMAGE);
        
            for (IWizardContributor contributor : entry.getValue()) {
                if (contributor instanceof IDiagramWizardContributor) {
                    diagramSubCategory.getContributors().add(contributor);
                } else {
                    matrixSubCategory.getContributors().add(contributor);
                }
            }
        
            if (!diagramSubCategory.getContributors().isEmpty()) {
                category.getSubCategories().add(diagramSubCategory);
            }
        
            if (!matrixSubCategory.getContributors().isEmpty()) {
                category.getSubCategories().add(matrixSubCategory);
            }
        }
    ...
    public Category getCategoryOfContributor(IWizardContributor contributor) {
