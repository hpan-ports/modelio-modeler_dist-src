...
class NSUseAnalyser extends AbstractModelVisitor {
    ...
    public Object visitBindableInstance(BindableInstance bi) {
        ...
        final ModelElement representedFeature = bi.getRepresentedFeature();
        if (representedFeature != null) {
            NameSpace src = NSUseUtils.getNameSpaceOwner(bi);
            NameSpace dest = NSUseUtils.getNameSpaceOwner(representedFeature);
    ...
    @objid ("24d84253-de11-458d-91ab-97fe783d1410")
    @Override
    public Object visitBpmnCallActivity(BpmnCallActivity theCallActivity) {
        Behavior behavior = theCallActivity.getCalledBehavior();
        if (behavior != null) {
            NameSpace src = NSUseUtils.getNameSpaceOwner(theCallActivity);
            NameSpace dest = NSUseUtils.getNameSpaceOwner(behavior);
        
            addNSUses(src, dest, theCallActivity);
        }
        
        BpmnTask task = theCallActivity.getCalledGlobalTask();
        if (task != null) {
            NameSpace src = NSUseUtils.getNameSpaceOwner(theCallActivity);
            NameSpace dest = NSUseUtils.getNameSpaceOwner(task);
        
            addNSUses(src, dest, theCallActivity);
        }
        
        Operation operation = theCallActivity.getCalledOperation();
        if (operation != null) {
            NameSpace src = NSUseUtils.getNameSpaceOwner(theCallActivity);
            NameSpace dest = NSUseUtils.getNameSpaceOwner(operation);
        
            addNSUses(src, dest, theCallActivity);
        }
        
        BpmnProcess process = theCallActivity.getCalledProcess();
        if (process != null) {
            NameSpace src = NSUseUtils.getNameSpaceOwner(theCallActivity);
            NameSpace dest = NSUseUtils.getNameSpaceOwner(process);
    ...
    public Object visitCallBehaviorAction(CallBehaviorAction theCallBehaviorAction) {
        Behavior calledBehavior = theCallBehaviorAction.getCalled();
        if (calledBehavior != null) {
            Activity activity = theCallBehaviorAction.getOwner();
            if (activity != null) {
                addNSUses(activity.getOwner(), calledBehavior.getOwner(), theCallBehaviorAction);
            }
        }
        return null;
    }
    ...
    public Object visitDependency(Dependency dep) {
        ...
        NameSpace src1 = NSUseUtils.getNameSpaceOwner(src0);
        NameSpace dest1 = NSUseUtils.getNameSpaceOwner(dest0);
    ...
    public Object visitMessage(Message theMessage) {
        ...
        if (op != null) {
            ...
            if (src != null) {
                addNSUses(src, op.getOwner(), theMessage);
            }
        }
        return null;
    }
    ...
    public Object visitProvidedInterface(ProvidedInterface theProvidedInterface) {
        final Port providingItf = theProvidedInterface.getProviding();
        if (providingItf != null) {
            NameSpace src = NSUseUtils.getNameSpaceOwner(providingItf);
    ...
    public Object visitRequiredInterface(RequiredInterface theRequiredInterface) {
        final Port requiring = theRequiredInterface.getRequiring();
        if (requiring != null) {
            NameSpace src = NSUseUtils.getNameSpaceOwner(requiring);
            if (src != null) {
    ...
    public Object visitTemplateParameter(TemplateParameter theTemplateParameter) {
        ...
        if (src != null) {
            ...
            if (type != null) {
                NameSpace dest = NSUseUtils.getNameSpaceOwner(type);
                addNSUses(src, dest, theTemplateParameter);
            }
        
            final ModelElement defaultType = theTemplateParameter.getDefaultType();
            if (defaultType != null) {
                NameSpace dest = NSUseUtils.getNameSpaceOwner(defaultType);
                addNSUses(src, dest, theTemplateParameter);
            }
        }
        return null;
    ...
    public Object visitTemplateParameterSubstitution(TemplateParameterSubstitution theTemplateParameterSubstitution) {
        final TemplateBinding owner = theTemplateParameterSubstitution.getOwner();
        final ModelElement actual = theTemplateParameterSubstitution.getActual();
        if (owner != null && actual != null) {
            addNSUses(NSUseUtils.getNameSpaceOwner(owner), NSUseUtils.getNameSpaceOwner(actual), theTemplateParameterSubstitution);
        }
        return null;
    }
