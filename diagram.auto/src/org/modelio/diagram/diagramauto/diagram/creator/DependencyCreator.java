...
public class DependencyCreator extends AbstractDiagramCreator {
    ...
    private void initialUnmasking(final IDiagramHandle dh, final NameSpace main) {
        ...
        for (NamespaceUse blueLink : main.getUserNsu()) {
            // Unmask user node
            NameSpace parent = blueLink.getUser();
        ...
        for (NamespaceUse blueLink : main.getUsedNsu()) {
            // Unmask used node
            NameSpace parent = blueLink.getUsed();
            List<IDiagramGraphic> nodes = dh.unmask(parent, 0, 0);
            if ((nodes != null) && (nodes.size() > 0)) {
                IDiagramNode node = (IDiagramNode) nodes.get(0);
    ...
    public void initStyle(final ModelTree main, final ModelTree owner, final IDiagramNode node) {
        if (main.equals(node.getElement())) {
            ...
            node.setStyle(StyleConstants.getStyleHandle(StyleConstants.EXTERN));
        }
    }
