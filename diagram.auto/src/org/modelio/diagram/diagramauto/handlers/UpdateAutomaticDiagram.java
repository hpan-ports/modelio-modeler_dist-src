...
public class UpdateAutomaticDiagram {
        ...
        try (ITransaction transaction = projectService.getSession().getTransactionSupport()
            ...
            for (MObject selectedElement : selectedElements) {
                if (selectedElement instanceof AbstractDiagram) {
                    final AbstractDiagram diagram = (AbstractDiagram) selectedElement;
    ...
    public boolean isEnabled(@Named(IServiceConstants.ACTIVE_SELECTION) final Object selection, IMModelServices modelServices) {
        ...
        for (MObject elt : selectedElements) {
            if (elt instanceof AbstractDiagram) {
                ...
                if (getCorrespondingDiagramCreator((AbstractDiagram) elt, modelServices) == null) {
                    return false;
                }
            } else {
                return false;
    ...
    public IDiagramCreator getCorrespondingDiagramCreator(AbstractDiagram existingdiagramauto, IMModelServices modelServices) {
        ...
        if (mainElement != null && mainElement instanceof Classifier && existingdiagramauto.equals(cac.getExistingAutoDiagram(mainElement))) {
            return cac;
        }
        
        ClassStructureCreator csc = new ClassStructureCreator(modelServices);
        mainElement = csc.getMainElement(existingdiagramauto);
        if (mainElement != null && mainElement instanceof Classifier && existingdiagramauto.equals(csc.getExistingAutoDiagram(mainElement))) {
            return csc;
        }
        ...
        if (mainElement != null && mainElement instanceof Classifier && existingdiagramauto.equals(ic.getExistingAutoDiagram(mainElement))) {
            return ic;
        }
        ...
        if (mainElement != null && mainElement instanceof Package && existingdiagramauto.equals(pcsc.getExistingAutoDiagram(mainElement))) {
            return pcsc;
        }
        ...
        if (mainElement != null && mainElement instanceof Package && existingdiagramauto.equals(spsc.getExistingAutoDiagram(mainElement))) {
            return spsc;
        }
        return null;
    }
