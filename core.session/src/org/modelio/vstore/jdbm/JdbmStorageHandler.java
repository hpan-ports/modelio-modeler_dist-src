...
class JdbmStorageHandler implements IRepositoryObject {
    ...
    private void setDirty(SmObjectImpl obj) {
        synchronized (this.repo) {
            if (! isDirty(obj)) {
                obj.getData().setRFlags(IRStatus.REPO_DIRTY, StatusState.TRUE);
    ...
    public boolean isDepLoaded(SmObjectImpl obj, SmDependency dep) {
        synchronized (this.repo) {
            if (isPersistent(dep))
                ...
                /*for (MRef ref : this.repo.getIndex().getUserRefs(new MRef(obj))) {
                SmObjectImpl val = this.repo.findById(SmClass.getClass(ref.mc), ref.uuid);
                if (!val.getRepositoryObject().isDepLoaded(val, dep.getSymetric()))
                    return false;
                }
                return true;*/
            }
    ...
    public boolean isDirty(SmObjectImpl obj) {
        synchronized (this.repo) {
            return obj.getData().hasAllStatus(IRStatus.REPO_DIRTY) == StatusState.TRUE;
        }
    }
