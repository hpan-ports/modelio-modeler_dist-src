...
public class MoveUpHelper extends AbstractHelper {
    ...
    public boolean execute(ModelElement element, List<ModelElement> selectedItems) {
        ...
        if (selectedItems.get(0) instanceof Note) {
            listToReorder = element.getDescriptor(); // list of Note
        } else if (selectedItems.get(0) instanceof Constraint) {
            listToReorder = element.getConstraintDefinition(); // list of Constraint
        } else if (selectedItems.get(0) instanceof ExternDocument) {
            listToReorder = element.getDocument(); // List of ExternDocument
        }
        
        for (ModelElement e : selectedItems) {
            int index = getIndexUp(e, listToReorder);
            if (index != -1) {
                nbToMove++;
