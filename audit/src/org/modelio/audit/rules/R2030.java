...
public class R2030 extends AbstractRule {
    ...
    public void autoRegister(IAuditPlan plan) {
        ...
        plan.registerRule(EnumeratedPropertyType.MNAME, this, AuditTrigger.CREATE |
                                                                       AuditTrigger.MOVE |
                                                                       AuditTrigger.UPDATE);
    }
    ...
    public static class CheckR2030 extends AbstractControl {
        ...
        public IDiagnosticCollector doRun(IDiagnosticCollector diagnostic, MObject element) {
            if (element instanceof PropertyContainer)
                diagnostic.addEntry(checkR2030((PropertyContainer) element));
            else if (element instanceof EnumeratedPropertyType) {
                PropertyContainer owner = ((EnumeratedPropertyType) element).getAnalystOwner();
                if (owner != null)
                    diagnostic.addEntry(checkR2030(owner));
            } else
                Audit.LOG.warning(Audit.PLUGIN_ID,
                            "R2030: unsupported element type '%s'",
                            element.getMClass().getName());
            return diagnostic;
        }

        @objid ("b199f77c-dc60-40e4-9159-ee0370033497")
        private IAuditEntry checkR2030(final PropertyContainer propertyContainer) {
