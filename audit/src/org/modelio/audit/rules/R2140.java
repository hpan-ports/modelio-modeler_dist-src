...
public class R2140 extends AbstractRule {
    ...
    public void autoRegister(IAuditPlan plan) {
        ...
        plan.registerRule(PropertyType.MNAME, this, AuditTrigger.CREATE |
                                                             AuditTrigger.MOVE |
                                                             AuditTrigger.UPDATE);
    }
    ...
    public static class CheckR2140 extends AbstractControl {
        ...
        public IDiagnosticCollector doRun(IDiagnosticCollector diagnostic, MObject element) {
            if (element instanceof PropertyContainer)
                diagnostic.addEntry(checkR2140((PropertyContainer) element));
            else if (element instanceof PropertyType) {
                PropertyContainer owner = ((PropertyType) element).getAnalystOwner();
                if (owner != null)
                    diagnostic.addEntry(checkR2140(owner));
            } else
                Audit.LOG.warning(Audit.PLUGIN_ID,
                            "R2140: unsupported element type '%s'",
                            element.getMClass().getName());
            return diagnostic;
        }

        @objid ("5f3fe329-68b6-4285-bf89-979af9f18154")
        private IAuditEntry checkR2140(final PropertyContainer propertyContainer) {
