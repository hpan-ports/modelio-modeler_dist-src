...
public class R2880 extends AbstractRule {
    ...
    public void autoRegister(IAuditPlan plan) {
        ...
        plan.registerRule(UseCaseDependency.MNAME + UseCaseDependencyIncludeKind, this, AuditTrigger.CREATE | AuditTrigger.MOVE | AuditTrigger.UPDATE);
    }
    ...
    public static class CheckR2880 extends AbstractControl {
        ...
        public IDiagnosticCollector doRun(IDiagnosticCollector diagnostic, MObject element) {
            if (element instanceof UseCase)
                ...
                if (dependency.isStereotyped("ModelerModule", UseCaseDependencyIncludeKind))
                    diagnostic.addEntry(checkR2880(dependency.getTarget()));
            } else
                Audit.LOG.warning("R2880: unsupported element type '%s'", element.getMClass().getName());
            return diagnostic;
        }
        ...
        private boolean checkCycle(UseCase useCase, List<UseCase> foundUseCases) {
            ...
            for (UseCaseDependency dep : useCase.getUsed()) {
                if (dep.isStereotyped("ModelerModule", UseCaseDependencyIncludeKind)) {
                    UseCase depTarget = dep.getTarget();
                    if (!targets.contains(depTarget)) targets.add(depTarget);
                }
            }
