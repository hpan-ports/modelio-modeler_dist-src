...
public class R1000 extends AbstractRule {
    ...
    public void autoRegister(IAuditPlan plan) {
        plan.registerRule(Abstraction.MNAME, this, AuditTrigger.CREATE | AuditTrigger.UPDATE);
    }

    /**
     * Default implementation is using a singleton for the checker. An alternative implementation consists in creating a
     * new instance of the checker for each element to check. This allows for fine tuning of the check depending on the
     * element status or on some external conditions. Use the 'new instance' strategy only if fine tuning of the check
     * is required for each element, because this strategy creates many objects (performance issues).
     */
    ...
    public static class CheckR1000 extends AbstractControl {
        ...
        public IDiagnosticCollector doRun(IDiagnosticCollector diagnostic, MObject element) {
            if (element instanceof Abstraction)
                diagnostic.addEntry(this.checkR1000((Abstraction) element));
            else
        ...
        private IAuditEntry checkR1000(Abstraction abstraction) {
            AuditEntry auditEntry = new AuditEntry(RULEID, AuditSeverity.AuditSuccess, abstraction, null);
