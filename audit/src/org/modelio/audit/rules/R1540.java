...
public class R1540 extends AbstractRule {
    ...
    public static class CheckR1540 extends AbstractControl {
        ...
        private IAuditEntry checkR1540(final BindableInstance bindableInstance) {
            ...
            // BindableInstance bindableinstance = (BindableInstance)element;
            
            // Code the test here and update 'auditentry' according to the test results
            //---------- BEGIN Cxx original code -----------------
            /*
            
                if (!object.belongTo(BindableInstanceClass()))
                    return;
                BindableInstance *currentInstance = dynamic_cast < BindableInstance *>(&object);
                ModelElement *representedFeature = NULL;
            
                std::vector < BindableInstance * > instances; // in case we detect a cycle, these are the related elements
                bool hasCycle = false;
                while (currentInstance && (!hasCycle))
                {
                    if (currentInstance->card_RepresentedFeature())
                    {
                        representedFeature = currentInstance->get_RepresentedFeature(0);
                        if (representedFeature->belongTo(BindableInstanceClass()))
                        {
                            if (representedFeature == &object)
                            {
                                hasCycle = true;
                            }
                            else
                            {
                                currentInstance = dynamic_cast < BindableInstance *>(representedFeature);
                ...
                if (hasCycle)
                {
                    ...
                    std::vector < BindableInstance * >::iterator it;
