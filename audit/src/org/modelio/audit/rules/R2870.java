...
public class R2870 extends AbstractRule {
    ...
    public void autoRegister(IAuditPlan plan) {
        ...
        plan.registerRule(UseCaseDependency.MNAME + UseCaseDependencyExtendKind, this, AuditTrigger.CREATE | AuditTrigger.MOVE | AuditTrigger.UPDATE);
    }
    ...
    public static class CheckR2870 extends AbstractControl {
        ...
        public IDiagnosticCollector doRun(IDiagnosticCollector diagnostic, MObject element) {
            if(element instanceof UseCase)
                ...
                if (dependency.isStereotyped("ModelerModule", UseCaseDependencyExtendKind))
                    diagnostic.addEntry(checkR2870(dependency.getTarget()));
            }
            else
                Audit.LOG.warning("R2870: unsupported element type '%s'", element.getMClass().getName());
            return diagnostic;
        }
        ...
        private boolean checkCycle(UseCase useCase, List<UseCase> foundUseCases) {
            if(foundUseCases.contains(useCase))
                return false;
            else foundUseCases.add(useCase);
            
            for(UseCaseDependency dep : useCase.getUsed()){
                if(dep.isStereotyped("ModelerModule", UseCaseDependencyExtendKind))
