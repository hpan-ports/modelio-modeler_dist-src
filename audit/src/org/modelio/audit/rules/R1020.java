...
public class R1020 extends AbstractRule {
    ...
    public static class CheckR1020 extends AbstractControl {
        ...
        public IDiagnosticCollector doRun(IDiagnosticCollector diagnostic, MObject element) {
            if (element instanceof ActivityNode) {
                ...
                Audit.LOG.warning(Audit.PLUGIN_ID,
                            "R1020: unsupported element type '%s'",
        ...
        private IAuditEntry checkR1020(final ActivityNode source, final ActivityNode target, final Element element) {
            ...
            if (source != null && target != null) {
                StructuredActivityNode sourceOwner = this.getOwningStructuredActivityNode(source);
                StructuredActivityNode targetOwner = this.getOwningStructuredActivityNode(target);
                ...
                if (source instanceof InputPin) {
                    InputPin source_as_input_pin = (InputPin) source;
                    if (source_as_input_pin.getInputing() != null) {
                        if (source_as_input_pin.getInputing().equals(targetOwner)) {
                            return auditEntry;
                        }
                    }
                }
                ...
                if (target instanceof OutputPin) {
                    OutputPin target_as_output_pin = (OutputPin) target;
                    if (target_as_output_pin.getOutputing() != null) {
                        if (target_as_output_pin.getOutputing().equals(sourceOwner))
                            return auditEntry;
                    }
                }
