...
public class R1080 extends AbstractRule {
    ...
    public void autoRegister(IAuditPlan plan) {
        // Activity
        plan.registerRule(ActivityPartition.MNAME, this,
                AuditTrigger.UPDATE | AuditTrigger.MOVE);
        plan.registerRule(Activity.MNAME, this, AuditTrigger.UPDATE);
        
        // BindableInstance
        plan.registerRule(Port.MNAME, this, AuditTrigger.MOVE);
    }
    ...
    /**
     * This checker is instantiated for a move event on a ActivityPartition or
     * on a Port. In this case, we only need to check the homogeneity of the
     * siblings partitions.
     */
    @objid ("3617f70a-9876-4e32-8642-c786018216c3")
    public class CheckR1080Move extends AbstractControl {
        ...
        public IDiagnosticCollector doRun(IDiagnosticCollector diagnostic, MObject element) {
            if (element instanceof Port)
                ...
                Audit.LOG.warning(Audit.PLUGIN_ID, "R1080: unsupported element type '%s'",
         ...
         * @return True is the partition's siblings partitions are homogeneous,
         * false otherwise
         */
         ...
         * @param partition The partition to check
         * @return True is the activity's children are homogeneous or if there
         * are no children, false otherwise
         */
        ...
        /**
         * Check if a list of partitions are homogeneous (that they represent
         * parts of the same Classifier)
        ...
        /**
         * Check if a partition needs to be checked by the rule (if it's not
         * external and if it's represented by a part.
         * @param partition The partition to check.
        ...
        /**
         * Checks is the Classifier is represented in any partition, and check
         * the rule on these partitions.
         * @param classifier
        ...
        private List<IAuditEntry> checkR1080(Port port) {
            ...
            if (((ModelElement) port).getRepresentingPartition() != null)
            
                // The classifier is represented in one or more partition, we
                // need to check the rule on each of these partitions.
            
                for (ActivityPartition partition : ((ModelElement) port)
    ...
    /**
     * This checker is instantiated either for an update on an ActivityPartityon
     * or an Activity.
     */
    @objid ("f3983126-97f1-4de9-8895-cee253c61b2f")
    public class CheckR1080Update extends CheckR1080Move {
        ...
        /**
         * In this case we need to check the siblings partition of our updated
         * partition, and the children partitions in case the partition was
         * updated after a moved partition event.
         * @param partition The partition to check
