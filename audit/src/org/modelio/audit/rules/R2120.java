...
public class R2120 extends AbstractRule {
    ...
    public void autoRegister(IAuditPlan plan) {
        plan.registerRule(PropertyContainer.MNAME, this, AuditTrigger.UPDATE);
        plan.registerRule(PropertyTableDefinition.MNAME, this, AuditTrigger.CREATE |
                                                            AuditTrigger.MOVE |
                                                            AuditTrigger.UPDATE);
    }
    ...
    public static class CheckR2120 extends AbstractControl {
        ...
        public IDiagnosticCollector doRun(IDiagnosticCollector diagnostic, MObject element) {
            if (element instanceof PropertyContainer)
                diagnostic.addEntry(checkR2120((PropertyContainer) element));
            else if (element instanceof PropertyTableDefinition) {
                PropertyContainer owner = ((PropertyTableDefinition) element).getOwner();
                if (owner != null)
                    diagnostic.addEntry(checkR2120(owner));
            } else
                Audit.LOG.warning(Audit.PLUGIN_ID,
                            "R2120: unsupported element type '%s'",
                            element.getMClass().getName());
            return diagnostic;
        }

        @objid ("a0aeed49-b027-47d3-a3b0-8ddc1c9841c1")
        private IAuditEntry checkR2120(final PropertyContainer propertyContainer) {
