...
public class R1310 extends AbstractRule {
    ...
    public void autoRegister(IAuditPlan plan) {
        ...
        plan.registerRule(InstanceNode.MNAME, this, AuditTrigger.UPDATE);
        
        plan.registerRule(ControlNode.MNAME, this, AuditTrigger.UPDATE);
        
        plan.registerRule(ObjectFlow.MNAME, this, AuditTrigger.CREATE | AuditTrigger.MOVE | AuditTrigger.UPDATE);
    }
    ...
    public static class CheckR1310 extends AbstractControl {
        ...
        public IDiagnosticCollector doRun(IDiagnosticCollector diagnostic, MObject element) {
            if (element instanceof InstanceNode)
                ...
                Audit.LOG.warning(Audit.PLUGIN_ID, "R1310: unsupported element type '%s'", element.getMClass().getName());
            return diagnostic;
        }

        /**
         * Checks if the given edge respect the rule: All downstream nodes must have an upper bound lower that the
         * flow's weight.
        ...
        /**
         * If a node is modified, its upper bound is potentially modified, so we need to check all the upstream flows'
         * weight, either object or control, targeting it or leading to it. A node can also be updated if a flow is
         * moved or deleted, potentially creating or removing paths between object nodes, so we need to check both
         * upstream and downstream path to updated concerned flows.
        ...
        /**
         * If an object flow is created or moved, it potentially connects object nodes together, and therefore creates
         * potentially new paths (in both direction) to check the rule on. If the object flow is updated, we also need
         * to check the rule on it.
        ...
        private List<IAuditEntry> checkR1310(ObjectFlow objectFlow) {
            ...
            if ((sourceNode instanceof ControlNode) ||
                (sourceNode instanceof InstanceNode && ((InstanceNode) sourceNode).isIsControlType())) {
            ...
            if (!(targetNode instanceof InstanceNode) ||
                (targetNode instanceof InstanceNode && ((InstanceNode) targetNode).isIsControlType())) {
        ...
        /**
         * Finds all the flows connecting the given ObjectNode to another ObjectNode, except for ObjectNodes that are
         * controls.
         * @param objectFlows
         * The list of found flows
