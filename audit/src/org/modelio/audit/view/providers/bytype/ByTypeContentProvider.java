...
public class ByTypeContentProvider implements ITreeContentProvider {
    ...
    public Object[] getElements(Object inputElement) {
        ...
        List<IAuditEntry> entrys = diagnostic.getEntries(this.jobId);
        
            this.error.entrys.clear();
            this.warning.entrys.clear();
            this.infos.entrys.clear();
        
            for(int i = 0; i< entrys.size();i++){
                IAuditEntry entry = entrys.get(i);
                if(entry.getSeverity().equals(AuditSeverity.AuditError)){
                    this.error.entrys.add(entry);
                }else if(entry.getSeverity().equals(AuditSeverity.AuditWarning)){
                    this.warning.entrys.add(entry);
                }else if(entry.getSeverity().equals(AuditSeverity.AuditAdvice)){
                    this.infos.entrys.add(entry);
                }
             }
    ...
    public Object[] getChildren(Object parentElement) {
        if(parentElement instanceof AuditTypeModel){
            return  ((AuditTypeModel)parentElement).entrys.toArray();
        }
        return null;
    }
    ...
    public boolean hasChildren(Object element) {
        return element instanceof AuditTypeModel && ((AuditTypeModel)element).entrys.size() > 0;
    }
