...
public class CreateSiblingElementHandler {
    ...
    @objid ("6f19350b-d108-41d2-a174-cd91e56301ef")
    @Inject
    @Optional
    ...
    public final boolean canExecute(@Named(IServiceConstants.ACTIVE_SELECTION) final Object selection) {
        ...
        List<MObject> toSiblings = getSelectedElements(selection);
        if (toSiblings.isEmpty()) {
            return false;
        }
        
        MObject dest = getPasteTarget(toSiblings);
        if (dest == null) {
            return false;
        }
    ...
    public final void execute(@Named(IServiceConstants.ACTIVE_SELECTION) final Object selection, Display currentDisplay) {
        final List<MObject> toSiblings = getSelectedElements(selection);
    ...
    private void reportException(Exception e) {
        // Show an error box
        String title = ModelBrowser.I18N.getMessage("CannotCreateSiblingElement");
