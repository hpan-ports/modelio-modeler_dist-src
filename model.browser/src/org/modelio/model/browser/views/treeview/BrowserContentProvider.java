...
import org.modelio.core.ui.labelprovider.ArchiveContainer;
import org.modelio.core.ui.labelprovider.LinkContainer;
...
public class BrowserContentProvider implements IModelChangeListener, IStatusChangeListener, ITreeContentProvider {
     ...
     * If {@link #findLinksChildren} or {@link #findArchivedVersionChildren} is <code>true</code>, the implementation should use
     * {@link #addResults(List)} so that children tree nodes are linked to the container. <br/>
    ...
    private static class ModelBrowserVisitor extends DefaultModelVisitor {
        /**
         * Do we show history versions (for Analyst Items)
         */
        ...
        @objid ("55e42074-35c2-4f3f-aa03-15ba6b0ebcd2")
        private Comparator<MObject> standardSorter = new StandardMetaclassSorter();
        ...
        public Object visitAnalystProject(AnalystProject theAnalystProject) {
            if (this.findScopeChildren) {
                ...
                // Risks
                addResults(theAnalystProject.getRiskRoot());
        ...
        public Object visitBpmnBehavior(final BpmnBehavior theBpmnBehavior) {
            if (this.findUmlChildren) {
                List<BpmnRootElement> rootElement = new ArrayList<>(theBpmnBehavior.getRootElement());
                Collections.sort(rootElement, this.standardSorter);
                addResults(rootElement);
            }
        ...
        public Object visitBpmnLane(final BpmnLane theBpmnLane) {
            if (this.findUmlChildren) {
                ...
                List<BpmnFlowElement> flowElementRef = new ArrayList<>();
                for (BpmnFlowElement elt : theBpmnLane.getFlowElementRef()) {
                    if (!(elt instanceof BpmnSequenceFlow)) {
                        flowElementRef.add(elt);
                    }
                }
                Collections.sort(flowElementRef, this.standardSorter);
                addResults(flowElementRef);
            }
        ...
        public Object visitBpmnProcess(final BpmnProcess theBpmnProcess) {
            if (this.findUmlChildren) {
                ...
                List<BpmnFlowElement> flowElements = new ArrayList<>();
                for (final BpmnFlowElement element : theBpmnProcess.getFlowElement()) {
                    if (!(element instanceof BpmnSequenceFlow) && element.getLane().isEmpty() && !(element instanceof BpmnBoundaryEvent)) {
                        flowElements.add(element);
                    }
                }
                Collections.sort(flowElements, this.standardSorter);
                addResults(flowElements);
            
                // BpmnArtifact
                addResults(theBpmnProcess.getArtifact());
        ...
        public Object visitBpmnSubProcess(final BpmnSubProcess theBpmnSubProcess) {
            if (this.findUmlChildren) {
                List<BpmnFlowElement> flowElement = new ArrayList<>();
                for (BpmnFlowElement elt : theBpmnSubProcess.getFlowElement()) {
                    if (!(elt instanceof BpmnSequenceFlow)) {
                        flowElement.add(elt);
                    }
                }
                Collections.sort(flowElement, this.standardSorter);
                addResults(flowElement);
            }
        ...
        public Object visitModelElement(ModelElement theModelElement) {
            if (this.findLinksChildren) {
                List<Dependency> dependsOnDependency = new ArrayList<>(theModelElement.getDependsOnDependency());
                Collections.sort(dependsOnDependency, this.standardSorter);
                addResults(dependsOnDependency);
            }
            if (this.findUmlChildren) {
        ...
        public Object visitModuleComponent(ModuleComponent theModule) {
            if (this.findUmlChildren) {
                ...
                addResults(theModule.getDefinedPropertyType());
        ...
        @objid ("b2f5dc5a-42f0-432f-ab75-a03ba4d524dc")
        @Override
        public Object visitRisk(Risk theRisk) {
            if (this.findScopeChildren) {
                // Risk
                for (Risk requirement : theRisk.getSubRisk()) {
                    addResult(requirement);
                }
            }
            ...
            // Old versions
            if (this.findArchivedVersionChildren) {
                addResults(theRisk.getArchivedRiskVersion());
            }
            return super.visitRisk(theRisk);
        }

        @objid ("3e342aa2-b396-4787-a3a4-43491884643c")
        @Override
        public Object visitRiskContainer(RiskContainer theRiskContainer) {
            if (this.findScopeChildren) {
                // RiskContainer
                for (RiskContainer requirementContainer : theRiskContainer.getOwnedContainer()) {
                    addResult(requirementContainer);
                }
            
                // Risk
                for (Risk requirement : theRiskContainer.getOwnedRisk()) {
                    addResult(requirement);
                }
            }
            return super.visitRiskContainer(theRiskContainer);
        }
