...
class SystemResizeCommand extends Command {
    ...
    SystemResizeCommand(final GraphicalEditPart freeZoneEditPart, final GraphicalEditPart child, final ChangeBoundsRequest request) {
    ...
    public void execute() {
        ...
        if (resizeParentCommand != null && resizeParentCommand.canExecute()) {
            TranslateChildrenCommand command = new TranslateChildrenCommand(this.freeZoneEditPart,
                                                                            moveDelta.getNegated());
            command.execute();
