...
public class UseCaseDiagramLayoutEditPolicy extends DiagramEditLayoutPolicy {
    ...
    protected Command getCreateCommand(final CreateRequest request) {
        ...
        if (UseCase.class == ctx.getJavaClass()) {
            ...
            MObject diagramContext = ((UseCaseDiagram) diagramGm.getRepresentedElement()).getOrigin();
            MObject elementToUnmask = ctx.getElementToUnmask();
            boolean systemShouldBeUsed = diagramContext != null &&
                                         diagramGm.getStyle()
                                                  .getBoolean(GmUseCaseDiagramStyleKeys.SHOW_SYSTEM);
            if (systemShouldBeUsed &&
                (elementToUnmask == null || isInSystem(diagramContext, elementToUnmask))) {
                ...
                if (systemGm.getLayoutData() == null || !systemGm.isVisible()) {
                    ...
                    ChangeBoundsRequest sizeRequest = new ChangeBoundsRequest(REQ_RESIZE);
    ...
    protected Command createAddCommand(final ChangeBoundsRequest request, final EditPart child, final Object constraint) {
        ...
        if (childModel instanceof GmUseCase) {
            ...
            GmUseCaseDiagram diagramGm = (GmUseCaseDiagram) getHost().getModel();
            MObject diagramContextElement = ((UseCaseDiagram) diagramGm.getRepresentedElement()).getOrigin();
            boolean systemShouldBeUsed = diagramContextElement != null &&
                                         diagramGm.getStyle()
                                                  .getBoolean(GmUseCaseDiagramStyleKeys.SHOW_SYSTEM);
            if (useCaseElement != null && systemShouldBeUsed && isInSystem(diagramContextElement, useCaseElement)) {
             // UseCase must be in the system boundaries.
                ...
                if (systemGm.getLayoutData() == null || !systemGm.isVisible()) {
                    ...
                    ChangeBoundsRequest moveRequest = new ChangeBoundsRequest(REQ_MOVE_CHILDREN);
