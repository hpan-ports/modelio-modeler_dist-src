...
/**
 * The 'BySet' model displays the IDiagramSet hierarchy of the project. The root node is the root diagram set named as the project
 * along with a unique VirtualFolder 'all diagrams' that itself displays all the diagrams as a flat list.
 */
...
public class BySetContentProvider implements ITreeContentProvider {
    ...
    public Object[] getElements(Object inputElement) {
        if (inputElement instanceof GProject) {
            ...
            GProject gProject = (GProject) inputElement;
            for (DiagramSet p : gProject.getSession().getModel().findByClass(DiagramSet.class, IModel.NODELETED)) {
                IProjectFragment fragment = gProject.getFragment(p);
    ...
    public Object[] getChildren(Object parent) {
        // the children of a diagram set are its sub-set and its diagram references
        if (parent instanceof DiagramSet) {
            ...
            for (MObject root : ((IProjectFragment) parent).getRoots()) {
                if (root instanceof Project) {
                    diagramSets.add(((Project) root).getDiagramRoot());
                }
            }
    ...
    public boolean hasChildren(Object parent) {
        ...
        if (parent instanceof DiagramRef) {
            ...
            return ((Project) parent).getDiagramRoot() != null;
        }
