...
public class ScriptView {
    ...
    public void createPartControl(Composite parent, MApplication application) {
        ...
        this.font = new Font(parent.getDisplay(), "Courier New", 10, SWT.NORMAL);
        ...
        Runnable onFlush = new Runnable() {
            ...
            public void run() {
                // TOODO popup view
                // getSite().getPage().activate(ScriptView.this);
            }
        };
        
        this.outputWriter = new PrintWriter(new BufferedWriter(new OutputWriter(this.outputView, new TextStyle(null, Display
                .getDefault().getSystemColor(SWT.COLOR_DARK_GREEN), null))));
        this.errorWriter = new PrintWriter(new BufferedWriter(new OutputWriter(this.outputView, new TextStyle(null, Display
                .getDefault().getSystemColor(SWT.COLOR_RED), null), onFlush)));
        this.commandWriter = new PrintWriter(new BufferedWriter(new OutputWriter(this.outputView, new TextStyle(null, Display
                .getDefault().getSystemColor(SWT.COLOR_BLACK), null))));
        
        this.inputView = new InputView(this.shform, SWT.BORDER | SWT.H_SCROLL | SWT.V_SCROLL, this);
        // this.inputView.getControl().setLayout(new FillLayout());
        // this.inputView.getControl().setFont(font);
        
        this.shform.setWeights(new int[] { 70, 30 });
        
        // Activate the key binding context for ScriptView
        // -----------------------------------------------
        // IContextService contextService = (IContextService)
        // getViewSite().getService(IContextService.class);
        // contextService.activateContext(ScriptMacro.ContextID);
    ...
    /**
     * Binding the jython runner to the current project consists in: - binding the python session variables - setting a classloader
     * that is aware of the currently started project modules classes
     * @param project
     */
    @objid ("007c43ae-52bf-106c-80fa-001ec947cd2a")
    private void bindJythonRunner(GProject project) {
        // @SuppressWarnings("resource")
        // TO REMOve ClassLoader classLoader = new ScriptClassLoader(this.moduleService);
