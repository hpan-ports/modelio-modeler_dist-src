...
public class MessageDragTracker extends SelectEditPartTracker {
    ...
    /**
     * Erases feedback and sets the viewer's focus to <code>null</code>. This will remove any focus rectangles that were
     * painted to show the new target or source edit part.
     * @see Tool#deactivate()
     */
    ...
    /**
     * Returns a custom "plug" cursor if this tool is in the initial, drag or accessible drag state. Otherwise defers to
     * <code>super</code>.
     * @return the cursor
     */
     ...
     * {@link ToolUtilities#getSelectionWithoutDependants(EditPartViewer)} .
    ...
    /**
     * Asks each edit part in the {@link AbstractTool#getOperationSet() operation set} to contribute to a
     * {@link CompoundCommand} after first setting the request type to {@link RequestConstants#REQ_MOVE}.
    ...
    /**
     * If currently in the drag-in-progress state, it goes into the terminal state erases feedback and executes the
     * current command.
    ...
    /**
     * Processes the arrow keys (to choose a different source or target edit part) and forwardslash and backslash keys
     * (to try to connect to another connection).
    ...
    /**
     * If auto scroll (also called auto expose) is being performed, the start location moves during the scroll. This
     * method updates that location.
     */
    ...
    /**
     * Calls {@link #repairStartLocation()} in case auto scroll is being performed. Updates the request with the current
     * {@link AbstractTool#getOperationSet() operation set}, move delta, location and type.
