 ...
 * @author fpoyer
 */
@objid ("d980a852-55b6-11e2-877f-002564c97630")
public class SequenceElementDropEditPolicy extends DiagramElementDropEditPolicy {
    /**
     * <p>
     * Overrides the default drop handling policy so that in some case it searches for the parent of the dropped element
     * (eg: for pins).
     * </p>
     */
    @objid ("d980a856-55b6-11e2-877f-002564c97630")
    @Override
    protected EditPart getDropTargetEditPart(final ModelElementDropRequest request) {
        ...
        for (MObject droppedElement : request.getDroppedElements()) {
            if (!gmModel.canUnmask(droppedElement)) {
                ...
                if (!isSmartLifelineNodeTarget(droppedElement, request) &&
                    !(droppedElement instanceof Interaction && request.isSmart())) {
     ...
     * </p>
     */
    @objid ("d980a860-55b6-11e2-877f-002564c97630")
    @Override
    protected Command getDropCommand(final ModelElementDropRequest request) {
        ...
        for (MObject toUnmask : request.getDroppedElements()) {
            ...
            if (isSmartLifelineNodeTarget(toUnmask, request)) {
                command.add(getSmartObjectNodeDropCommand(dropLocation, toUnmask));
            } else if (toUnmask instanceof Interaction && request.isSmart()) {
                command.add(getSmartInteractionDropCommand(dropLocation, toUnmask));
    ...
    private Command getSmartObjectNodeDropCommand(final Point dropLocation, final MObject toUnmask) {
        ...
        if (origin instanceof Interaction) {
            return new SmartCreateLifelineCommand(dropLocation, toUnmask, getHost(), origin);
        } else {
    ...
    @objid ("d980a873-55b6-11e2-877f-002564c97630")
    private boolean isSmartLifelineNodeTarget(final MObject element, final ModelElementDropRequest request) {
        if (!request.isSmart())
            return false;
    ...
    private Command getSmartInteractionDropCommand(final Point dropLocation, final MObject toUnmask) {
        final GmSequenceDiagram diagramModel = (GmSequenceDiagram) getHost().getModel();
        final SequenceDiagram owner = (SequenceDiagram) diagramModel.getRelatedElement();
        Interaction origin = (Interaction) owner.getOrigin();
        return new SmartCreateInteractionUseCommand(dropLocation, (Interaction) toUnmask, getHost(), origin);
    }

    @objid ("d9822ee1-55b6-11e2-877f-002564c97630")
