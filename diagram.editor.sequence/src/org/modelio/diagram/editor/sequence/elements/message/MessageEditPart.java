...
/**
 * EditPart (ie Controller in the GEF model) for Message in sequence diagram. Note: for lack of a better place, this
 * editpart installs the move policy on itself (no good "parent" to do it).
 */
...
public class MessageEditPart extends AbstractConnectionEditPart implements PropertyChangeListener, NodeEditPart {
    ...
    protected IFigure createFigure() {
        ...
        MessageSort sort = this.getLinkModel().getSort();
    ...
    protected final void swapEnd(final MObject newEndElement, final ReconnectRequest request) {
        ...
        List<GmModel> models = this.getLinkModel().getDiagram().getAllGMRelatedTo(new MRef(newEndElement));
        ...
        if (!foundUnmaskedEnd) {
            ...
            if (request.getType().equals(REQ_RECONNECT_SOURCE)) {
                dropPoint = ((GraphicalEditPart) this.getSource()).getFigure()
                                                                  .getBounds()
                                                                  .getLocation()
                                                                  .getTranslated(20, 20);
            } else {
        ...
        models = this.getLinkModel().getDiagram().getAllGMRelatedTo(new MRef(newEndElement));
        for (GmModel model : models) {
            // For each gm, search the corresponding edit part
            EditPart editPart = (EditPart) this.getViewer().getEditPartRegistry().get(model);
            if (editPart != null) {
    ...
    /**
     * Returns the <code>ConnectionAnchor</code> for the specified <i>source</i> connection. This NodeEditPart is the
     * {@link ConnectionEditPart#getSource() source} EditPart for the given connection.
     * <P>
    ...
    /**
     * Returns the <i>source</i> <code>ConnectionAnchor</code> for the specified Request. The returned ConnectionAnchor
     * is used only when displaying <i>feedback</i>. The Request is usually a
    ...
    /**
     * Returns the <code>ConnectionAnchor</code> for the specified <i>target</i> connection. This NodeEditPart is the
     * {@link ConnectionEditPart#getTarget() target} EditPart for the given connection.
     * <P>
     * The anchor may be a function of the connection's model, the node's model, a combination of both, or it may not
     * depend on anything all.
     * @param connection the ConnectionEditPart
    ...
    /**
     * Returns the <i>target</i> <code>ConnectionAnchor</code> for the specified Request. The returned ConnectionAnchor
     * is used only when displaying <i>feedback</i>. The Request is usually a
     * {@link org.eclipse.gef.requests.LocationRequest}, which provides the current mouse location.
     * @param request a Request describing the current interaction
     * @return the ConnectionAnchor to use during feedback
