 ...
 * <p>
 * Computes whether the move is valid, gives feedback to the user and could also
 * compute the command.
 * <p>
 * Usage:<ul>
 * <li> to be instantiated in {@link org.eclipse.gef.EditPolicy#activate()} or lazilly.
 * <li> from  {@link org.eclipse.gef.EditPolicy#showSourceFeedback(org.eclipse.gef.Request)} and
 * {@link org.eclipse.gef.EditPolicy#getCommand(org.eclipse.gef.Request)} (directly or indirectly) :<ul>
 * <li> call {@link #computePredicatesForHost(MObject...)} for all involved model elements
 * <li> call {@link #updateVariable(MObject, int)}/{@link #updateVariable(TimeReference, int)}
 * on each model element to be moved.
 * <li> then call {@link #showFeedBack(IFigure)} or {@link #checkAllPredicates()} to know whether the move is valid.
 * </ul>
 * <li> from  {@link org.eclipse.gef.EditPolicy#eraseSourceFeedback(org.eclipse.gef.Request)} ,
 * call {@link #eraseFeedback(IFigure)}.
 * <li> in some cases {@link #eraseFeedback(IFigure)} must be called from {@link org.eclipse.gef.EditPolicy#showSourceFeedback(org.eclipse.gef.Request)}
 * when the request type changes from REQ_MOVE to REQ_ADD.
 * </ul>
 */
...
public class ManipulationHelper {
    ...
    private List<IFigure> fbRectangles = new ArrayList<>();
    ...
    private void computePredicatesForMessageEnd(final MessageEnd messageEnd) {
        ...
        if (messageEnd instanceof ExecutionOccurenceSpecification) {
            ...
            if (eosDot.getStarted() != null) {
                ExecutionOccurenceSpecification otherEnd = eosDot.getStarted()
                        .getFinish();
            ...
            if (eosDot.getFinished() != null) {
                ExecutionOccurenceSpecification otherEnd = eosDot.getFinished()
                        .getStart();
                TimeReference otherEndTimeReference = new TimeReference(otherEnd);
                Variable variable2 = initVariable(otherEndTimeReference, otherEnd.getLineNumber());
     ...
     * <p>
     * The variable is created if non existent.
     * If the variable already exists it is directly returned.
     * @param ref the time reference
     * @param value the new line value
     * @return the found or created variable.
     */
