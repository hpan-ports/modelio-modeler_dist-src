...
public class GmLifelineBody extends GmNoStyleCompositeNode {
    ...
    public GmLifelineBody(final GmAbstractDiagram diagram, final MRef relatedRef) {
    ...
    public boolean canUnmask(final MObject el) {
        return (el instanceof InteractionFragment) &&
    ...
    public void refreshFromObModel() {
        ...
        if (relatedMObject != null && relatedMObject.isValid()) {
            ...
            Collections.sort(executionSpecifications, new Comparator<ExecutionSpecification>() {
                @Override
                public int compare(ExecutionSpecification o1, ExecutionSpecification o2) {
                    if (o1.getStart().getLineNumber() < o2.getStart().getLineNumber()) {
                        return -1;
                    } else if (o1.getStart().getLineNumber() == o2.getStart().getLineNumber()) {
                        return 0;
                    } else {//if (o1.getStart().getLineNumber() > o2.getStart().getLineNumber()) {
                        return 1;
                    }
            ...
            Collections.sort(stateInvariants, new Comparator<StateInvariant>() {
                @Override
                public int compare(StateInvariant o1, StateInvariant o2) {
            ...
            Collections.sort(executionOccurenceSpecifications,
                             new Comparator<ExecutionOccurenceSpecification>() {
                                 @Override
                                 public int compare(ExecutionOccurenceSpecification o1,
                                         ExecutionOccurenceSpecification o2) {
                                     if (o1.getLineNumber() < o2.getLineNumber()) {
                                         return -1;
                                     } else if (o1.getLineNumber() == o2.getLineNumber()) {
                                         return 0;
    ...
    public void read(IDiagramReader in) {
        ...
        Object versionProperty = in.readProperty("GmLifelineBody." + MINOR_VERSION_PROPERTY);
    ...
    public void write(IDiagramWriter out) {
        ...
        if (this.minorVersion != 0) {
            out.writeProperty("GmLifelineBody." + MINOR_VERSION_PROPERTY, Integer.valueOf(this.minorVersion));
        }
    }
