...
public class GAuthConf {
    ...
    public static GAuthConf from(AuthDescriptor desc) {
        if (desc == null || desc.getScheme() == null || desc.getScheme().isEmpty())
    ...
    public boolean reconfigure(AuthDescriptor authDescriptor) {
        ...
        // Do nothing if empty descriptor, unless it is a shared one
        if (!authDescriptor.isDefined() && authDescriptor.getScope() != DefinitionScope.SHARED)
            return false;
