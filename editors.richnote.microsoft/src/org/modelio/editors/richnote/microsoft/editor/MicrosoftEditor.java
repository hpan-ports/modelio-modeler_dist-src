...
public class MicrosoftEditor implements IRichNoteEditor {
    ...
    public void doSave(final IProgressMonitor monitor) {
        ...
        try {
            if (element instanceof Artifact) {
                ...
                ExternDocument doc = (ExternDocument) element;
    ...
    private String getProgramId(final ExternDocument doc) {
    ...
    void initTitle() {
        ...
        if (el == null)
            return ;
        else if (el instanceof ExternDocument)
            title = RichNoteLabelProvider.getLabel((ExternDocument) el);
        else
            title = el.getName();
        ...
        if (el==null)
            ...
            Image ss = RichNoteLabelProvider.getIcon((ExternDocument) el);
        } else {
            StandardImageService.getStereotypedImage(el, null);
        }
        */
    }
    ...
    private void loadExternDocument(final ExternDocument doc) throws IOException {
    ...
    /**
     * Show/hide Word menu bar in place of the Modelio menu bar when the
     ...
     * <p>
     * Runs periodically in the SWT thread.
     * Sets the {@link MPart#setDirty(boolean) dirty} property when the dirty state change.
     */
    ...
    private class ModelListener implements IModelChangeListener, IStatusChangeListener {
        ...
        public void dispose() {
            final IModelChangeSupport imodel = this.minput.getSession().getModelChangeSupport();
