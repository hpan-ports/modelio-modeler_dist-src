...
public class GmOperationGroup extends GmGroup {
    ...
    public GmOperationGroup(GmAbstractDiagram diagram, MRef relatedRef) {
    ...
    public boolean isVisible() {
        if (getParent().getRepresentationMode().equals(RepresentationMode.STRUCTURED))
            return getStyle().getProperty(getGroupVisibleStyleKey());
        return false;
    }
    ...
    public void refreshFromObModel() {
        ...
        if (classifier != null && classifier.isValid()) {
            StyleKey filterStyleKey = getFilterStyleKey();
            if (filterStyleKey != null) {
                final StyleKey.UmaskByVisibilityStragegy mode = this.getStyle().getProperty(filterStyleKey);
                switch (mode) {
                    case ALL:
    ...
    protected void doSetVisible(boolean visible) {
        if (visible) {
            getParent().getStyle().setProperty(getStyleKey(MetaKey.REPMODE), RepresentationMode.STRUCTURED);
        }
        getStyle().setProperty(getGroupVisibleStyleKey(), visible);
    }
    ...
    protected boolean isValidElement(MObject el) {
        ...
        StyleKey.UmaskByVisibilityStragegy unmaskmode = getStyle().getProperty(getFilterStyleKey());
        switch (unmaskmode) {
            case ALL:
                return true;
            case ALL_PUBLIC:
    ...
    protected void updateHiddenFeatures() {
        final Classifier classifier = (Classifier) this.getRelatedElement();
        if (classifier != null && classifier.isValid()) {
            boolean hasHiddenFeature = false;
        
            final StyleKey.UmaskByVisibilityStragegy mode = this.getStyle().getProperty(getFilterStyleKey());
            switch (mode) {
                case ALL:
                    break;
    ...
    public void read(IDiagramReader in) {
        ...
        Object versionProperty = in.readProperty("GmOperationGroup." + MINOR_VERSION_PROPERTY);
    ...
    public void write(IDiagramWriter out) {
        ...
        if (this.minorVersion != 0) {
            out.writeProperty("GmOperationGroup." + MINOR_VERSION_PROPERTY, Integer.valueOf(this.minorVersion));
        }
    }
