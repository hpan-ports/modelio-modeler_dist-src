...
public class GmInnerClassGroup extends GmGroup {
    ...
    public GmInnerClassGroup(GmAbstractDiagram diagram, MRef relatedRef) {
    ...
    public boolean isVisible() {
        ...
        if (parent.getRepresentationMode().equals(RepresentationMode.STRUCTURED) && parent.isVisible()) {
    ...
    public void refreshFromObModel() {
        ...
        if (classifier != null && classifier.isValid()) {
            ...
            if (unmaskFilterStyleKey != null) {
                final StyleKey.UmaskByVisibilityStragegy mode = this.getStyle()
                                                                    .getProperty(unmaskFilterStyleKey);
                switch (mode) {
                    case ALL:
    ...
    protected void doSetVisible(boolean visible) {
        if (visible) {
            getStyle().setProperty(getViewModeStyleKey(), InternalsViewMode.LIST);
            ((GmInnerClass) getParent()).setVisible(true);
        } else
            getStyle().setProperty(getViewModeStyleKey(), InternalsViewMode.NONE);
    }
    ...
    protected boolean isValidElement(MObject el) {
        ...
        final StyleKey.UmaskByVisibilityStragegy unmaskmode = getStyle().getProperty(getUnmaskFilterStyleKey());
        switch (unmaskmode) {
            case ALL:
                return true;
            case ALL_PUBLIC:
    ...
    protected void updateHiddenFeatures() {
        ...
        final StyleKey.UmaskByVisibilityStragegy mode = this.getStyle().getProperty(unmaskFilterStyleKey);
        final NameSpace classifier = (NameSpace) getRelatedElement();
        if (classifier != null && classifier.isValid()) {
    ...
    public void read(IDiagramReader in) {
        ...
        Object versionProperty = in.readProperty("GmInnerClassGroup." + MINOR_VERSION_PROPERTY);
    ...
    public void write(IDiagramWriter out) {
        ...
        if (this.minorVersion != 0) {
            out.writeProperty("GmInnerClassGroup." + MINOR_VERSION_PROPERTY,
