...
public class GmInstanceInternalStructureGroup extends GmGroup {
    ...
    public GmInstanceInternalStructureGroup(GmAbstractDiagram diagram, MRef relatedRef) {
    ...
    public boolean isVisible() {
        if (getParent().getRepresentationMode().equals(RepresentationMode.STRUCTURED)) {
            InternalsViewMode v = getStyle().getProperty(GmInstanceStructuredStyleKeys.INTERNALSVIEWMODE);
    ...
    public void refreshFromObModel() {
        ...
        if (instance != null && instance.isValid()) {
            final Boolean autoShow = this.getStyle()
                                         .getProperty(GmInstanceStructuredStyleKeys.InternalStructure.AUTOUNMASK);
            if (autoShow) {
    ...
    protected void doSetVisible(boolean visible) {
        if (visible) {
            getStyle().setProperty(GmInstanceStructuredStyleKeys.INTERNALSVIEWMODE, InternalsViewMode.LIST);
            ((GmInstanceInternalStructure) getParent()).setVisible(true);
        } else {
            getStyle().setProperty(GmInstanceStructuredStyleKeys.INTERNALSVIEWMODE, InternalsViewMode.NONE);
        }
    }
    ...
    protected void updateHiddenFeatures() {
        final Instance instance = (Instance) getRelatedElement();
        if (instance != null && instance.isValid()) {
            Boolean autoShow = this.getStyle()
                                   .getProperty(GmInstanceStructuredStyleKeys.InternalStructure.AUTOUNMASK);
    ...
    public void read(IDiagramReader in) {
        ...
        Object versionProperty = in.readProperty("GmInstanceInternalStructureGroup." + MINOR_VERSION_PROPERTY);
    ...
    public void write(IDiagramWriter out) {
        ...
        if (this.minorVersion != 0) {
            out.writeProperty("GmInstanceInternalStructureGroup." + MINOR_VERSION_PROPERTY, Integer.valueOf(this.minorVersion));
        }
    }
