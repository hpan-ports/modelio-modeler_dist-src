...
public class GmPackageMigrator implements IPersistentMigrator {
    ...
    private IPersistent migrateFromV0(final IPersistent instanceToMigrate) {
        _GmPackage oldPackage = (_GmPackage) instanceToMigrate;
        ...
        GmPackagePrimaryNode newPrimaryNode = newPackage.getMainNode();
        ...
        newPackage.getStyle().setCascadedStyle(oldPackage.getStyle().getCascadedStyle());
        for (StyleKey key : oldPackage.getStyle().getLocalKeys()) {
            newPackage.getStyle().setProperty(key, oldPackage.getStyle().getProperty(key));
        }
