...
public final class GmInnerClass extends GmNoStyleCompositeNode implements PropertyChangeListener {
    ...
    public GmInnerClass(final GmAbstractDiagram diagram, final MRef relatedRef) {
        super(diagram, relatedRef);
        
        this.innerZone = new GmInnerClassesZone(diagram, relatedRef);
        ...
        this.innerGroup = new GmInnerClassGroup(diagram, relatedRef);
    ...
    public GmInnerClass(final GmAbstractDiagram diagram, final MRef relatedRef, final GmFreeZone zone, final GmGroup group) {
        super(diagram, relatedRef);
    ...
    public void read(final IDiagramReader in) {
        ...
        Object versionProperty = in.readProperty("GmInnerClass." + MINOR_VERSION_PROPERTY);
    ...
    public void write(final IDiagramWriter out) {
        ...
        if (this.minorVersion != 0) {
            out.writeProperty("GmInnerClass." + MINOR_VERSION_PROPERTY, Integer.valueOf(this.minorVersion));
        }
    }
    ...
    public GmCompositeNode getCompositeFor(final Class<? extends MObject> metaclass) {
        if (getStyle().getProperty(getStyleKey(MetaKey.InnerGroup.INNERVIEWMODE)) == InternalsViewMode.DIAGRAM) {
    ...
    private boolean isManualUnmask() {
        ...
        if (styleKey != null) {
            final StyleKey.UmaskByVisibilityStragegy mode = this.getStyle().getProperty(styleKey);
    ...
    public boolean isVisible() {
        if (getParent().getRepresentationMode().equals(RepresentationMode.STRUCTURED)) {
            ...
            InternalsViewMode v = getStyle().getProperty(viewModeStyleKey);
