...
public class GmCentralBufferMigrator implements IPersistentMigrator {
    ...
    private IPersistent migrateFromV0(final IPersistent instanceToMigrate) {
        _GmCentralBuffer oldCentralBuffer = (_GmCentralBuffer) instanceToMigrate;
        ...
        GmCentralBufferPrimaryNode newPrimaryNode = newCentralBuffer.getMainNode();
        ...
        newCentralBuffer.getStyle().setCascadedStyle(oldCentralBuffer.getStyle().getCascadedStyle());
        for (StyleKey key : oldCentralBuffer.getStyle().getLocalKeys()) {
            newCentralBuffer.getStyle().setProperty(key, oldCentralBuffer.getStyle().getProperty(key));
        }
