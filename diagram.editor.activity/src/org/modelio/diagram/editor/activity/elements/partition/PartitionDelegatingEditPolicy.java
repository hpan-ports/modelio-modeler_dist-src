...
/**
 * Specialisation of the default delegating policy to handle the Partition specific tools like "create sibling partition",
...
public class PartitionDelegatingEditPolicy extends DelegatingEditPolicy {
    /**
     * If the request is to create a sibling partition, then delegate to the partition container currently holding the host. If the
     * request is to create an inner partition, then delegate to the partition container currently held by the host. Request for
    ...
    public EditPart getTargetEditPart(Request request) {
        if (RequestConstants.REQ_ADD.equals(request.getType()) || RequestConstants.REQ_MOVE.equals(request.getType())) {
            ...
            if (((ChangeBoundsRequest) request).getEditParts().get(0) instanceof PartitionEditPart) {
