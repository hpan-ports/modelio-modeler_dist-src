...
public class PExportProfile implements IExportProfileElement {
    ...
    public void visit() {
        org.eclipse.uml2.uml.Profile ecoreElt = (org.eclipse.uml2.uml.Profile) GenerationProperties.getInstance().getMappedElement(objingElt);
        
        if (ecoreElt == null){
            ecoreElt = ProfileUtils.createEcoreProfile(objingElt);  
    ...
    private void setProperties(org.eclipse.uml2.uml.Profile ecoreElt) {
        ObjingEAnnotation.setModule(ecoreElt, objingElt.getOwnerModule().getUuid().toString());
        ObjingEAnnotation.addObjingID(ecoreElt, objingElt.getUuid().toString());
        String name = ProfileUtils.getName(objingElt);
        ecoreElt.setName(name);
    }

    @objid ("d24f3069-05b5-42b5-8877-a31f8ea480a1")
    public List<PExportProfile> getSubProfiles() {
        List<PExportProfile> subProfiles = new ArrayList<PExportProfile>();
        
        for (ModelTree sub : objingElt.getOwnedElement()){
    ...
    public List<PExportStereotype> getStereotypes() {
        List<PExportStereotype> result = new ArrayList<PExportStereotype>();
        for (Stereotype sub : this.objingElt.getDefinedStereotype()){
            if (ObjingModelNavigation.mustBeExported(sub)){
    ...
    public List<PExportReference> getReferences() {
        List<PExportReference> result = new ArrayList<PExportReference>();
