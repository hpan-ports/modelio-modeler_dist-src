...
public class ProfileUtils {
    ...
    @objid ("5a6f82db-9cbd-4f9d-bd7d-3ba0f2af7148")
    ...
    private static List<String> getObjNameClass(final java.lang.Class<?> classe, final Method methodCreateElt) {
        ...
        try{
            ...
            for (Constructor<?> current : classe.getDeclaredConstructors()){
                if (current.getParameterTypes().length == 0)
                    createClasse= current;
            }
        
            methodCreateElt.invoke(org.eclipse.uml2.uml.UMLFactory.eINSTANCE, (Object[])null);
        
            Method methodIjingClass = classe.getMethod("getIjingClassName");
        
            if (createClasse != null) {
                Object classeInstance = createClasse.newInstance();
                result = (List<String>) methodIjingClass.invoke(classeInstance);
            }
    ...
    private static byte[] getBytesFromFile(final File file) throws IOException {
        ...
        //  org.eclipse.uml2.uml.Read in the bytes
        int offset = 0;
        int numRead = 0;
    ...
    public static void addExtension(final ModelElement objModelElement, final org.eclipse.uml2.uml.Stereotype stereotype) {
        if (!ObjingEAnnotation.isReference(stereotype)){
            ...
            //            List<Stereotype> existingStereotypes = Modelio.getInstance().getModelingSession().getMetamodelExtensions().getStereotypes(objModelElement.getMClass());
            //
            //            boolean notApply = true;
            //            for (Stereotype existingStereotype : existingStereotypes){
            //                if (importedStereotypes.contains(existingStereotype)){
            ...
            if ((notApply) && !(ObjingEAnnotation.isReference(stereotype))) {
                ...
                Stereotype objStereotype = Modelio.getInstance().getModelingSession().getMetamodelExtensions().getStereotype(
                        name, objModelElement.getMClass() );
        
                if (!objModelElement.getExtension().contains(objStereotype))
    ...
    private static Object createObjStereotype(final org.eclipse.uml2.uml.Stereotype ecoreElement) {
        ...
        List<String> ids = ObjingEAnnotation.getObjingIDs(ecoreElement);
        for (String id : ids){
            Stereotype result = (Stereotype) Modelio.getInstance().getModelingSession().findElementById(SmClass.getClass(Stereotype.class), id);
            if (result != null)
                results.add(result);        
        }
        ...
        if (results.size() == 0){
                ...
                for (org.eclipse.uml2.uml.Element ownedElt : ecoreElement.getOwnedElements()){
                    if (ownedElt instanceof Property ){
                        ...
                        if ((ecoreProperty.getAssociation() instanceof Extension)
                            ...
                            baseClasses.addAll(ProfileUtils.getObjingNameClass(typeName));
                        }
                    }
                }
            }
            ...
            for (String baseClass : baseClasses){
                ...
                Stereotype result = (Stereotype) Modelio.getInstance().getModelingSession().getModel().createElement("Stereotype");
    ...
    private static void setIcons(final Stereotype objElt, final org.eclipse.uml2.uml.Stereotype ecoreElement) {
        ...
        if (icons.size() > 0 ){
            if (ReverseProperties.getInstance().isRoundtripEnabled()){
                ...
                setIcon(objElt, (org.eclipse.uml2.uml.Image) icons.get(0), "icon");
                setIcon(objElt, (org.eclipse.uml2.uml.Image) icons.get(0), "explorerIcon");
                setIcon(objElt, (org.eclipse.uml2.uml.Image) icons.get(0), "smallIcon");
            }
    ...
    public static void write(final org.eclipse.swt.graphics.Image img, final String fileName, final String format) throws IOException {
    ...
    private static NoteType createNoteType(final Stereotype stereotype, final Property ecoreElement) {
        NoteType noteType  = (NoteType)  Modelio.getInstance().getModelingSession().getModel().createElement("NoteType");
    ...
    private static TagType createTagType(final Stereotype stereotype, final Property ecoreElement) {
        ...
        TagType attr = (TagType) Modelio.getInstance().getModelingSession().getModel().createElement("TagType");
    ...
    private static void createNoteType(final MetaclassReference reference, final Property ecoreElement) {
        ...
        if (noteType == null){
            noteType = (NoteType) Modelio.getInstance().getModelingSession().getModel().createElement("NoteType");
    ...
    private static TagType createTagType(final MetaclassReference reference, final Property ecoreElement) {
        ...
        if (ids.size() > 0){
            ...
            if ((name != null ) && (!name.equals("")) 
                    ...
                    && (tagType == null)){
                tagType = (TagType) Modelio.getInstance().getModelingSession().getModel().createElement("TagType");
                ...
                if ((!stereotype.getStatus().isRamc()) 
                    ...
                    if (ObjingEAnnotation.isNoteType(ecoreElement)){
                        results.add(createNoteType(stereotype, ecoreElement));
                            ...
                            && (!name.startsWith("base_")) && (ecoreElement.getType() != null) 
                            ){
    ...
    public static boolean isAnalystDeployed() {
        ...
        //            if (module.getUuid().toString().equals("87d35db0-2fb3-11e0-b88e-002564c97630"))
        //                return true;
        //        }
        return false;
    }
    ...
    private static List<TaggedValue> sortByType(ModelElement element) {
        ...
        List<TaggedValue> listTagSorted = new ArrayList<TaggedValue>();
