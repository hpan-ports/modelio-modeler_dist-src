...
class Controller {
    ...
    private boolean isValidPath(String path) {
        ...
        if (path.endsWith(RAMC_FILE_EXTENSION)) {
            return true;
        } else {
            return false;
        }
    ...
    public void onRemoveDependency(RamcModel ramcModel) {
        this.model.getRequiredModelComponents().remove(ramcModel);
    }
    ...
    public void onContributorChange(Object[] checkedElements) {
        this.model.getContributingModules().clear();
                for (Object o : checkedElements) {
                    IModule m = (IModule)o;
                    this.model.getContributingModules().put(m.getName(), m.getVersion().toString());
                }
    }
