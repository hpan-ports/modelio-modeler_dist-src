...
public class EditRamcDialog extends ViewRamcDialog {
    @objid ("597d2bb8-b237-442e-95fe-6ce6267f6e91")
    ...
    public void addButtonsInButtonBar(Composite parent) {
        ...
        this.packageButton = this.createButton(parent, IDialogConstants.FINISH_ID,
        ...
        this.dependenciesTable.getTable().addKeyListener(new KeyListener() {
            ...
            public void keyReleased(KeyEvent e) {
                if (e.keyCode == SWT.DEL) {
                    ...
                    TableItem[] items = table.getSelection();
                    for (TableItem item : items) {
                        EditRamcDialog.this.controller.onRemoveDependency((RamcModel) item.getData());
                    }
                }
                EditRamcDialog.this.refresh();
            }
    ...
    private void reconfigureFilesList() {
        ...
        /*
         * ramcFilesChooser.setFilterExtensions(new String[] {"*.ramc"});
         * ramcFilesChooser.setFilterNames(new String[]
         * {AppRamcs.I18N.getString("EditRamcDialog.RamcFiles")});
         */
        ...
        this.addFilesButton.addSelectionListener(new SelectionListener() {
            ...
            public void widgetSelected(SelectionEvent e) {
                ...
                for (String filename : filesChooser.getFileNames()) {
                    ...
                    String exportPath;
                    if (file.toString().startsWith(projectPath.toString())) {
                        // The selected file belongs to the project, deploy
                        // using the same path relative to the project
                        exportPath = file.toString().replace(projectPath.toString(), "");
                    } else {
                    ...
                    EditRamcDialog.this.dataModel.addExportedFile(new ExportedFileEntry(file.toString(), exportPath));
                }
        ...
        this.removeFilesButton.addSelectionListener(new SelectionListener() {
            ...
            @Override
            public void widgetSelected(SelectionEvent e) {
                Table table = EditRamcDialog.this.ramcFilesList.getTable();
                TableItem[] items = table.getSelection();
                for (TableItem item : items) {
                    ExportedFileEntry file = (ExportedFileEntry) item.getData();
                    EditRamcDialog.this.dataModel.removeExportedFile(file);
                }
                EditRamcDialog.this.refresh();
            }
        });
    ...
    private void reconfigureContributorsList() {
        ...
        this.contributorsTable.addCheckStateListener(new ICheckStateListener() {
            @Override
            public void checkStateChanged(CheckStateChangedEvent event) {
                EditRamcDialog.this.controller.onContributorChange(EditRamcDialog.this.contributorsTable.getCheckedElements());
            }
        });
    }
    ...
    private static class ManifestationsDropListener extends ViewerDropAdapter {
        ...
        private Controller controler = null;
         ...
         * @param model - access to the model in order to be able to find model
         * element from their MRef
         */
        ...
        /**
         * Indicates whether or not the currently transfered elements can be
         * dropped on the target.
         * <p>
         * Metamodel rules are checked, as well as manipulation rights for both
         * the target and dropped elements.
         * </p>
         * @param target the targeted element, must be a MObject.
         * @param operation the d&d operation, must be {@link DND#DROP_COPY} or
         * {@link DND#DROP_MOVE}.
        ...
        public boolean validateDrop(Object target, int operation, TransferData transferType) {
            ...
            if (refs != null) {
                ...
                for (int i = 0; i < refs.length; i++) {
                    dropedElements.add((Element) this.model.findByRef(refs[i]));
                }
            } else {
        ...
        /**
         * After a drop has been stated as valid, copy or move the dragged
         * elements under the target.
         */
        ...
        private boolean canBeExported(MObject element) {
            if (element instanceof org.modelio.metamodel.uml.statik.Package) {
                return !(element.getCompositionOwner() instanceof Project);
            } else if (element instanceof NameSpace) {
                return true;
            } else if (element instanceof AnalystContainer) {
                return true;
