...
public class ExternEditorsManager {
    ...
    void onElementEdition(@EventTopic(ModelioEventTopics.EDIT_ELEMENT) final MObject target) {
        ...
        if (target instanceof Artifact) {
            ...
            try {
                if (artifact.isStereotyped(MODULE_NAME, FILE) || artifact.isStereotyped(MODULE_NAME, DIRECTORY)) {
                ...
                if (artifact.isStereotyped(MODULE_NAME, URL)) {
                ...
                if (artifact.isStereotyped(MODULE_NAME, MAIL)) {
    ...
    private Path computePath(Artifact fileArtifact) {
        ...
        while (owner instanceof Artifact && owner.isStereotyped(MODULE_NAME, DIRECTORY)) {
    ...
    private URI computeUri(Artifact urlArtifact) throws URISyntaxException {
        return new URI(urlArtifact.getFileName());
    }
