...
public class IModuleLoadedFeature extends AbstractFeature {
    ...
    public void disable() {
        ...
        URLClassLoader classLoader = this.module.getClassLoader();
        if (classLoader != null) {
            try {
                classLoader.close();
            } catch (IOException e) {
    ...
    private void restartWeakDependentModules() {
        for (IRTModule aModule : this.module.getModuleOptionalUsers()) {
            aModule.refreshDependencies();
        
            ModuleRuntimeState currentState = aModule.getState();
        
            try {
                MdaInfra.LOG.debug(" reloading '%s' because optional '%s' is loaded.", aModule.getName(), this.module.getName());
