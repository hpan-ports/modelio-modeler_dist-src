...
import org.modelio.mda.infra.service.impl.controller.states.actions.CallModuleInstallAction;
import org.modelio.mda.infra.service.impl.controller.states.actions.SelectModuleAction;
import org.modelio.mda.infra.service.impl.controller.states.actions.SetGModuleStateAction;
import org.modelio.mda.infra.service.impl.controller.states.actions.UnselectModuleAction;
import org.modelio.mda.infra.service.impl.controller.states.features.DiagramStylesInstalledFeature;
import org.modelio.mda.infra.service.impl.controller.states.features.DocFeature;
import org.modelio.mda.infra.service.impl.controller.states.features.DynamicModelFeature;
import org.modelio.mda.infra.service.impl.controller.states.features.GuiContribFeature;
import org.modelio.mda.infra.service.impl.controller.states.features.IModuleLoadedFeature;
import org.modelio.mda.infra.service.impl.controller.states.features.MmFragmentsFeature;
import org.modelio.mda.infra.service.impl.controller.states.features.StartedFeature;
...
public class States {
    ...
    public States(IRTModuleAccess rtModule) {
        ...
        // Set states features
        // -------------------
        // Note: The features are activated in the order they are declared.
        //       Order is important because some depend on others
    ...
    private void add(org.modelio.mda.infra.service.impl.controller.states.Transition.Builder tb) {
    ...
    public void changeState(AbstractModuleState newState) throws ModuleException {
        MdaInfra.LOG.debug("'%s' module state changing from '%s' to '%s'", this.rtModule.getName(), this.currentState, newState);
        ...
        try {
            ...
            MdaInfra.LOG.debug("'%s' module state change to '%s' failed: %s",this.rtModule.getName(), newState, ex.getMessage());
        
            // Restore previous state
            try {
                this.currentState.enterState(oldState);
                this.rtModule.setDownError( ex);
            } catch (ModuleException | RuntimeException e2) {
                e2.addSuppressed(ex);
            ...
            this.currentState = oldState;
            MdaInfra.LOG.debug("'%s' module state change to '%s' unexpectedly failed: %s",this.rtModule.getName(), newState, ex);
    ...
    public void handleMessage(Object message) throws ModuleException {
        MdaInfra.LOG.debug("'%s' module [%s] : message '%s'", this.rtModule.getName(), this.currentState, message);
     ...
     * @param newState the new state
     * @throws org.modelio.api.module.ModuleException if state change is refused by a feature, in case of breaking error
     */
    ...
    private void updateFeatures(AbstractModuleState newState) throws ModuleException {
        ...
        for (ListIterator<IModuleFeature> it = this.currentFeatures.listIterator(this.currentFeatures.size());  it.hasPrevious(); ) {
            ...
            if (! newFeatures.contains(f)) {
                try {
                    if (DEBUG_FEATURES) {
                        MdaInfra.LOG.debug("'%s' module state [%s] : disabling feature '%s' ",this.rtModule.getName(),newState,f);
                    }
                    ...
                    // Log and continue
                    MdaInfra.LOG.debug("'%s' module state [%s] : disabling feature '%s' failed: %s",this.rtModule.getName(),newState,f,e.toString());
        ...
        try {
            for (IModuleFeature f : newFeatures) {
                ...
                if (! this.currentFeatures.contains(f)) {
                    if (DEBUG_FEATURES) {
                        MdaInfra.LOG.debug("'%s' module state [%s] : enabling feature '%s' ",this.rtModule.getName(),newState,f);
                    }
            ...
            if (! ok) {
                MdaInfra.LOG.debug("'%s' module state [%s] :  '%s' feature failed to enable, reverting features...  ",this.rtModule.getName(),newState, processed);
                MdaInfra.LOG.indent();
                ...
                for (IModuleFeature f : toAbort) {
                    try {
                        ...
                        MdaInfra.LOG.debug("'%s' module state [%s] : reverted feature '%s' ", this.rtModule.getName(), newState, f);
                    } catch (ModuleException e) {
                        MdaInfra.LOG.debug("'%s' module state [%s] : reverting feature '%s' failed: %s", this.rtModule.getName(), newState, f, e.toString());
                        MdaInfra.LOG.debug(e);
                    } catch (RuntimeException e) {
                        MdaInfra.LOG.warning("'%s' module state [%s] : reverting feature '%s' failed: %s", this.rtModule.getName(), newState, f, e.toString());
                        MdaInfra.LOG.warning(e);
                    }
                }
