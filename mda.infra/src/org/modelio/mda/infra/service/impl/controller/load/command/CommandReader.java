...
public class CommandReader {
    ...
    public IModuleAction createCommand(Jxbv2Command commandDef, IRTModule module) throws IOException {
        ...
        // Create the handler
        IModuleCommandHandler handler = helper.createHandler(ContributionReader.CONTRIB_COMMAND,
                commandDef.getHandler().getClazz(),IModuleCommandHandler.class);
        
        // Collect applicable scopes
        List<CommandScope> scopes = helper.readScopes(commandDef.getScope());
        
        // Collect hParameters
        Map<String, String> hParameters = helper.readParameters(commandDef.getHandler().getHParameter());
