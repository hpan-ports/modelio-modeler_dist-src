...
import org.modelio.mda.infra.service.ModuleRefusedActionException;
...
public class StartedFeature extends AbstractFeature {
    ...
    public void enable() throws ModuleException {
        ...
        // Start the module
        try {
            boolean ok = this.module.getIModule().getSession().start();
        
            if (! ok) {
                throw new ModuleRefusedActionException(MdaInfra.I18N.getMessage(
                        "StartedFeature.StartReturnedFalse", this.module.getLabel()));
            }
        } catch (RuntimeException | LinkageError e) {
            String message = MdaInfra.I18N.getMessage("ModuleStarter.startFailed",
                    this.module.getLabel(), e.toString());
            throw new ModuleException(message, e);
        }
        
        // Register the module for the image loader
        ModuleI18NService.registerModule(this.module.getModel(), this.module.getIModule());
    ...
    public void disable() throws ModuleException {
        ...
        this.module.getIModule().getSession().stop();
        
        // Unregister the module from the image loader
        ModuleI18NService.unregisterModule(this.module.getModel());
