...
class RTModule implements IRTModuleAccess {
    ...
    @objid ("bffb68b8-6b46-4092-b6c8-bd0311b48606")
    public List<IRTModule> getUsedModules() {
        return this.usedModules;
    }
    ...
    public final void refreshDependencies() {
        ...
        if (this.gmodule != null) {
            ...
            for (GModule strongDependency : requiredGModules) {
                ...
                this.requiredModules.add(rtDep);
    ...
    private void initModuleUsers() {
        this.moduleUsers = new ArrayList<>();
        this.moduleOptionalUsers = new ArrayList<>();
        
        if (this.gmodule != null) {
            for (IRTModule m : this.moduleRegistry.getModules()) {
                if (m.getRequiredDependencies().contains(this)) {
                    this.moduleUsers.add(m);
                }
        
                if (m.getOptionalDependencies().contains(this)) {
                    this.moduleOptionalUsers.add(m);
                }
            }
        }
    }
