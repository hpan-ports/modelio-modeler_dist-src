        ...
        if (RequestConstants.REQ_DIRECT_EDIT.equals(req.getType())) {
            final CellEditorLocator cellEditorLocator = new CellEditorLocator() {
                ...
                public void relocate(CellEditor cellEditor) {
                    final IFigure mainFig = getFigure();
                    final TextFigure label = getLabelFigure(mainFig);
                    final Rectangle rect = label.getBounds().getCopy();
                    final Dimension txtSize = TextUtilities.INSTANCE.getStringExtents("pPF", label.getFont());
                    final Dimension newSize = Dimension.max(rect.getSize(), new Dimension(mainFig.getSize().width - 10, txtSize.height));
                    rect.expand((newSize.width - rect.width) / 2.0, (newSize.height - rect.height) / 2.0);
