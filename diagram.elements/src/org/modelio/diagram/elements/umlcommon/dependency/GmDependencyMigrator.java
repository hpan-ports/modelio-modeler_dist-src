...
public class GmDependencyMigrator implements IPersistentMigrator {
    ...
    private IPersistent migrateFromV0(final IPersistent instanceToMigrate) {
        GmDependency oldDependency = (GmDependency) instanceToMigrate;
        ...
        newDependency.getStyle().setCascadedStyle(oldDependency.getStyle().getCascadedStyle());
        for (StyleKey key : oldDependency.getStyle().getLocalKeys()) {
            newDependency.getStyle().setProperty(key, oldDependency.getStyle().getProperty(key));
        }
