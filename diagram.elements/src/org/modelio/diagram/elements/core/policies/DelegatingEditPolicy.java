...
/**
 * Generic policy that delegates the {@link #getTargetEditPart(Request)} method call to another edit part (determined by a call to
 * the host's getCompositeFor method).
 * <p>
...
public class DelegatingEditPolicy implements EditPolicy {
    ...
    /**
     * Activates this EditPolicy. The EditPolicy might need to hook listeners. These listeners should be unhooked in
     * <code>deactivate()</code>. The EditPolicy might also contribute feedback/visuals immediately, such as <i>selection
    ...
    /**
     * Deactivates the EditPolicy, the inverse of {@link #activate()}. Deactivate is called when the <i>host</i> is deactivated, or
     * when the EditPolicy is uninstalled from an active host. Deactivate unhooks any listeners, and removes all feedback.
    ...
    /**
     * Erases source feedback based on the given <code>Request</code>. Does nothing if the EditPolicy does not apply to the given
     * Request.
     * <P>
     * This method is declared on {@link EditPart#eraseSourceFeedback(Request) EditPart}, and is redeclared here so that EditPart
     * can delegate its implementation to each of its EditPolicies.
    ...
    /**
     * Erases target feedback based on the given <code>Request</code>. Does nothing if the EditPolicy does not apply to the given
     * Request.
     * <P>
     * This method is declared on {@link EditPart#eraseTargetFeedback(Request) EditPart}, and is redeclared here so that EditPart
     * can delegate its implementation to each of its EditPolicies.
     * @param request the Request
     */
    ...
    /**
     * Returns the <code>Command</code> contribution for the given <code>Request</code>, or <code>null</code>. <code>null</code> is
     * treated as a no-op by the caller, or an empty contribution. The EditPolicy must return an
     * {@link org.eclipse.gef.commands.UnexecutableCommand} if it wishes to disallow the Request.
     * <P>
     * This method is declared on {@link EditPart#getCommand(Request) EditPart}, and is redeclared here so that EditPart can
     * delegate its implementation to each of its EditPolicies. The EditPart will combine each EditPolicy's contribution into a
    ...
    public EditPart getTargetEditPart(Request request) {
        if (RequestConstants.REQ_CREATE == request.getType()) {
            return getTargetEditPartForCreateRequest(request);
            ...
            return getTargetEditPartForGroupRequest(request);
        }
    ...
    private EditPart getTargetEditPartForGroupRequest(Request request) {
        ...
        EditPart ret = null;
        final GroupRequest gr = (GroupRequest) request;
    ...
    /**
     * Shows or updates <i>source feedback</i> for the specified <code>Request</code>. This method may be called repeatedly for the
     ...
     * <P>
     * This method is declared on {@link EditPart#showSourceFeedback(Request) EditPart}, and is redeclared here so that EditPart can
     * delegate its implementation to each of its EditPolicies.
     * @param request the Request
     */
    ...
    /**
     * Shows or updates <i>target feedback</i> for the specified <code>Request</code>. This method may be called repeatedly for the
     * purpose of updating feedback based on changes to the Request.
     * <P>
     * Does nothing if the EditPolicy does not recognize the given request.
     * <P>
     * This method is declared on {@link EditPart#showTargetFeedback(Request) EditPart}, and is redeclared here so that EditPart can
     * delegate its implementation to each of its EditPolicies.
     * @param request the Request
     */
     ...
     * <P>
     * This method is declared on {@link EditPart#understandsRequest(Request) EditPart}, and is redeclared here so that EditPart can
     * delegate its implementation to each of its EditPolicies. <code>EditPart</code> returns <code>true</code> if any of its
