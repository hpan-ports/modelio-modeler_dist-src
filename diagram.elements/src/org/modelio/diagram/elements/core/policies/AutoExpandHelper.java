...
public class AutoExpandHelper {
    ...
    public static Command getExpandContainerCommand(ChangeBoundsRequest request, EditPart containerEditPart, IFigure layoutContainer) {
        ...
        Dimension requiredSize = computePreferredContainerSize(request, containerEditPart, layoutContainer);
    ...
    public static ChangeBoundsRequest getNewChildAutoExpandRequest(EditPart containerEditPart, GraphicalEditPart newChild, IFigure layoutContainer) {
        ...
        Dimension curSize = layoutContainer.getSize();
        Dimension requiredSize = computeRequiredContainerSizeForNewChild(newChild, containerEditPart, layoutContainer);
    ...
    protected static Dimension computePreferredContainerSize(ChangeBoundsRequest request, EditPart containerEditPart, IFigure layoutContainer) {
        ...
        for (GraphicalEditPart child : containerChildren) {
            if (reqChildren.contains(child)) {
                ...
                Rectangle constraint = getNewBoundsFor(request, child);
    ...
    @objid ("02a6e332-4b14-4d4a-ac8e-dedf8f8d7a31")
    private static Rectangle getNewBoundsFor(Rectangle childConstraint, GraphicalEditPart child) {
        if (childConstraint.width() == -1 || childConstraint.height() == -1) {
            Dimension childPrefSize = child.getFigure().getPreferredSize();
            if (childConstraint.width() == -1) {
                childConstraint.width = childPrefSize.width();
            }
        
            if (childConstraint.height() == -1) {
                childConstraint.height = childPrefSize.height();
            }
        }
    ...
    protected static class TempSizeLayout extends ChainedLayout {
        ...
        public static LayoutManager restore(IFigure fig) {
            TempSizeLayout tempLayout = (TempSizeLayout) fig.getLayoutManager();
            LayoutManager old = tempLayout.getChained();
            fig.setLayoutManager(old);
            return old;
        }
