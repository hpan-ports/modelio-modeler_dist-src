...
/**
 * This is a geometric utility class that allows for manipulation of line segments. A line segment is defined as a set
 * of two points where one point is designated as the origin and the other is the terminal.
 * 
 * @author sshaw
 */
...
public class LineSeg implements Cloneable, java.io.Serializable, Translatable {
    ...
    /**
     * Returns the coefficients of the generalized equation of the line passing through points (x1,y1) and (x2,y2)
    ...
    /**
     * Creates a segment using (fromX, fromY) as either the first point of the segment (start == Origin) or the midpoint
     * of the segment (start == Midpoint), and using slope as its new slope and len as the new length. xdir indicates
     * which direction the segment should go in the x-axis.
     ...
     * @return <code>boolean</code> <code>true</code> if the given point lies on this segment, <code>false</code>
     * otherwise.
     */
    @objid ("7f836358-1dec-11e2-8cad-001ec947c8cc")
    public final boolean containsPoint(final Point aPoint, final int tolerance) {
        /*
         * We need perform the calculations in double numbers to avoid possible integer overflows in
         * Point#getDistance2() method
         */
    ...
    public final float distanceAlong(Point coord) {
         ...
         * 
         * To find out if point lies "inside" line segment (i.e. can draw perpendicular line from segment to point), use
         * projection of point (q) to line (p + td): t = (q-p).d/length(d)^2 (. is dot product)
         */
        ...
        /*
         * avoid divide by 0 - check if point1 equals point2. If so, there is no segment - return a value which
         * indicates projection falls outside the segment.
         */
        if (dirx == 0 && diry == 0)
            return -1;
        
        /*
         * length (magnitude) of d is sqrt(dirx^2 + diry^2). Don't bother taking square root since we want the length
         * squared.
         */
    ...
    /**
     * Finds the perpendicular distance from a point coordinates to this line segment. If point is "inside" line
     * segment, then use distance from point to the line, otherwise use distance to nearest endpoint of segment
    ...
    /**
     * Returns array with 3 numbers in it, which are the coefficients of the generalized line equation of the line
     * corresponding to this line segment a*x+b*y=c is the equation => result[0]=a, result[1]=b, result[2]=c
    ...
    /**
     * Calculates intersection points of the line that contains this line segment with a list of other line segments. If
     * the list of points (line segments) form a closed <Code>PolyLine</Code>, i.e form a closed polygon figure, then
     * the method will claculate intersections of a line and a figure
    ...
    /**
     * Returns intersection points of two lines that contain this line segment and the argumet line segment. The list of
     * intersection points may contain at most two points and will contain 2 points if and only if the lines are equal.
     * The 2 points will be the end points of the parameter line segment
    ...
    /**
     * Returns a new <code>LineSeg</code> that is parallel to this by the given distance. Orientation is relative to the
     * start and end. Negative implies to the left and Position implies to the right.
     ...
     * @return <code>TrigValues</code> object representing the trigonometry values for the angle of the passed in
     * <code>Ray</code> relative to <code>this</code> or null if calculation is not possible,
     */
    ...
    /**
     * Determines the intersect point between this line and the line passed in as a parameter. If they intersect, then
     * true is returned and the point reference passed in will be set to the intersect point. If they don't intersect,
     ...
     * @return <code>Point</code> that represents the intersection with this line, or <code>null</code> if the
    ...
    /**
     * Locates a point at a given height and distance along the line segment. B (the point we are looking for) + | dist
     * |h this segment P1-----------+-------------------> A get point A (on picture above)
    ...
    /**
     * Calculates the perpendicular slope of this line segment. This calculates the slope and then inverts it. Again, to
     * avoid divide by zero errors, the constant <code>BIGSLOPE</code> is returned if the calculated slope before
    ...
    /**
     * Returns out a positive or negative value (Positive / Negative) depending on the orientation of the given point to
     * the line. Point on this side: Positive. P1------------------------------> this segment Point on this side:
     * Negative.
     ...
     * @return <code>float</code> the slope of this segment. If the slope is not defined such as when the line segment
     * is vertical, then the constant <code>BIGSLOPE</code> is returned to avoid divide by zero errors.
     */
    ...
    /**
     * Enumeration class for defining the keypoint along a line segment. Can be one of <code>ORIGIN</code>,
     * <code>MIDPOINT</code> or <code>TERMINUS</code>.
     */
    ...
    /**
     * Enumeration class for defining the orientations of a point relative to the line segment. The orientations can be
     * one of <code>POSITIVE</code> or <code>NEGATIVE</code>.
     */
