...
class RotatedGraphics extends Graphics {
    ...
    public void clipPath(Path path) {
        ...
        g.clipPath(new Path(path.getDevice(), d));
    }
    ...
    public void drawArc(int x, int y, int w, int h, int offset, int length) {
        g.drawArc((int) tx(x, y), (int) ty(x,y), w, h, offset+angle, length+angle);
    }
    ...
    public void drawImage(Image srcImage, int x1, int y1, int w1, int h1, int x2, int y2, int w2, int h2) {
        g.pushState();
        g.rotate(angle);
        g.drawImage(srcImage, x1, y1, w1, h1, x2, y2, w2, h2);
        g.popState();
    }
    ...
    public void drawLine(int x1, int y1, int x2, int y2) {
        g.drawLine(tx(x1,y1), ty(x1,y1), tx(x2,y2), ty(x2,y2));
    }
    ...
    public void drawTextLayout(TextLayout layout, int x, int y, int selectionStart, int selectionEnd, Color selectionForeground, Color selectionBackground) {
        g.pushState();
        g.rotate(angle);
        g.drawTextLayout(layout, x, y, selectionStart, selectionEnd,
    ...
    public void fillArc(int x, int y, int w, int h, int offset, int length) {
        g.pushState();
        g.rotate(angle);
        g.fillArc(x, y, w, h, offset, length);
        g.popState();
    }
    ...
    public void fillGradient(int x, int y, int w, int h, boolean vertical) {
        g.pushState();
        g.rotate(angle);
        g.fillGradient(x, y, w, h, vertical);
        g.popState();
    }
    ...
    public void fillPolygon(int[] points) {
        g.fillPolygon(trPoints(points));
    }

    @objid ("8d826dbe-e181-4ad2-b0f5-81625540eeb9")
    @Override
    public void fillRectangle(int x, int y, int width, int height) {
        g.fillPolygon(trRectangle(x, y, width, height));
    }
    ...
    public float getLineMiterLimit() {
        return g.getLineMiterLimit();
    }

    @objid ("32a188fb-3b3e-452c-b913-8fd0866bd5ba")
    @Override
    public int getLineStyle() {
        return g.getLineStyle();
    }
    ...
    public float getLineWidthFloat() {
        return g.getLineWidthFloat();
    }

    @objid ("f26858f6-39cf-482c-9208-60e5a0869da1")
    @Override
    public int getTextAntialias() {
        return g.getTextAntialias();
    }
    ...
    public void setClip(Path path) {
        ...
        g.setClip(new Path(path.getDevice(), d));
    }
    ...
    public void setLineMiterLimit(float miterLimit) {
        g.setLineMiterLimit(miterLimit);
    }
    ...
    public void setLineWidthFloat(float width) {
        g.setLineWidthFloat(width);
    }

    @objid ("2e3fd068-25e4-4874-9984-d7a5b6b42d6b")
    @Override
    public void setTextAntialias(int value) {
        g.setTextAntialias(value);
    }
