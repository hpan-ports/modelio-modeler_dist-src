...
public class RoundedLinkFigure extends LinkFigure {
    ...
    private ArrayList<Connection> collectConnections() {
        final ArrayList<Connection> result = new ArrayList<>();
        collectConnections(getRoot(), result);
        return result;
    }

    /**
     * find all connection figures owned by the given figure.
     * @param figure a figure
     * @param list the found connections
     */
    @objid ("7fb0affb-1dec-11e2-8cad-001ec947c8cc")
    ...
    private void drawLine(Graphics g, Point pp, Point p1, ArrayList<Connection> connections) {
        if (this.bridgesEnabled && pp.x == p1.x) {
            ...
            if (segments.size() > 0) {
                ...
                int y = y1;
                for (Iterator<Integer> I = segments.iterator(); I.hasNext();) {
                    int y0 = I.next().intValue();
    ...
    private void outlineRoundedShape(Graphics g) {
        ...
        for (int i = 1; i < psCount; i++) {
            ...
            final Point pn = ps.getPoint(i + 1).getCopy();
