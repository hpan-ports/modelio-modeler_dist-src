...
public class LinearSlidableAnchor extends AbstractConnectionAnchor implements ISlidableAnchor {
    ...
    /**
     * Calculates the relative location of the reference point with respect to the bounds of the figure. If point p is
     * not inside of the figure's bounds then the point is mapped on the bounds and the point relative location is
     * calculated
     * @param bounds
     ...
     * @return <code>boolean</code> <code>true</code> is the <code>SlidableAnchor</code> is default one,
    ...
    /**
     * Calculates intersection points of the figure and the line that passes through ownReference and foreignReference
     * points
     * @param ownReference the reference <code>Point</code> on or inside the shape that is being anchored to.
     * @param foreignReference the outside reference <code>Point</code> point that is the terminal end of the line formed by the two
     * parameters.
     * @return intersection points of the figure and the line that passes through ownReference and foreignReference
     * points
     */
    ...
    /**
     * Returns the list of all the vertices of the figure. The created list must form a polygon, i.e. closed polyline,
     * for figures hence the starting and ending points must be the same
     ...
     * @param foreignReference <code>Point</code> that is the foreign reference point used to calculate the interfection anchor point
     * on the shape in absolute coordinates.
     * @param ownReference <code>Point</code> that is the reference point with-in the shape in absolute coordinates
     * @param tolerance <code>int</code> value that is the difference in absolute coordinates where the two points would be
     * considered straight and then adjusted.
     * @return <code>Point</code> that is the normalized owned reference to be with-in a given straight-line tolerance
     * value of the foreign reference point.
     */
    ...
    /**
     * From relative reference returns the relative coordinates of the anchor Method's visibility can be changed as
     * needed
     */
