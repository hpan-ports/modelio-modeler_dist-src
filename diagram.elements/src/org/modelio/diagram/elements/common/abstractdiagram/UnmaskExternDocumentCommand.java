...
public class UnmaskExternDocumentCommand extends Command {
    ...
    public UnmaskExternDocumentCommand(final ExternDocument theExternDocument, final AbstractDiagramEditPart host, final Object initialLayoutData, final Point dropLocation) {
        this.theExternDocument = theExternDocument;
        this.host = host;
    ...
    public void execute() {
        ...
        GmNodeModel targetModel = this.diagram.unmask(this.diagram, this.theExternDocument, this.constraint);
        ...
        req.setFactory(new ModelioLinkCreationContext(this.theExternDocument));
        req.setType(RequestConstants.REQ_CONNECTION_START);
        
        // Look for edit part of subject element... If none found, unmask it.
        ModelElement subject = this.theExternDocument.getSubject();
        ...
        // Unmask the link
        GmLink link = this.diagram.unmaskLink(this.theExternDocument);
    ...
    public boolean canExecute() {
        ...
        return MTools.getAuthTool().canModify(((GmAbstractDiagram) this.host.getModel()).getRelatedElement());
    }
