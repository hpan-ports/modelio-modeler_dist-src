...
public class CreateConstraintCommand extends Command {
    ...
    public void execute() {
        ...
        if (newElement == null) {
            ...
            final IModelFactory modelFactory = diagram.getModelManager().getModelFactory(this.sourceModels.get(0).getRelatedElement());
            newElement = modelFactory.createConstraint();
        
            // ... and attach it to it to all source models.
            for (GmModel sourceModel : this.sourceModels) {
                newElement.getConstrainedElement().add((ModelElement) sourceModel.getRelatedElement());
            }
