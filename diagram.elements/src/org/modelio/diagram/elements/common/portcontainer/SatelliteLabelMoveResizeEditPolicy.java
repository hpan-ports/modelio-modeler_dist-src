...
public class SatelliteLabelMoveResizeEditPolicy extends DefaultNodeResizableEditPolicy {
    ...
    protected ChangeBoundsRequest patchResizeRequest(ChangeBoundsRequest origRequest) {
        ...
        Rectangle askedBounds = fig.getBounds().getCopy();
        ...
        Dimension bestSize = computeBestSize(request, askedBounds);
        fig.translateToAbsolute(bestSize);
        
        Rectangle initBounds = fig.getBounds().getCopy();
    ...
    private void fixFigurePosition(ChangeBoundsRequest request, IFigure fig, Rectangle initBounds, IFigure refFigure) {
        if (true) {
            Rectangle refBounds = refFigure.getBounds().getCopy();
            ...
            case EAST:
                // stick on left
                request.setMoveDelta(new Point(0, - sizeDelta.height() / 2));
                break;
            case NONE:
                // Move the figure so that it has the same center point after resize
                request.setMoveDelta(new Point(- sizeDelta.width() / 2, - sizeDelta.height() / 2));
                break;
            case NORTH:
                request.setMoveDelta(new Point(- sizeDelta.width() / 2, - sizeDelta.height()));
                break;
            case SOUTH:
                request.setMoveDelta(new Point(- sizeDelta.width() / 2, 0));
                break;
            case WEST:
                request.setMoveDelta(new Point(- sizeDelta.width() , - sizeDelta.height() / 2));
                break;
            default:
                // Move the figure so that it has the same center point after resize
                request.setMoveDelta(new Point(- sizeDelta.width() / 2, - sizeDelta.height() / 2));
    ...
    private Dimension computeBestSize(ChangeBoundsRequest request, Rectangle askedBounds) {
        ...
        if (delta.height() == 0) {
            return fig.getPreferredSize(askedBounds.width(), -1).getCopy();
        } else if (delta.width() == 0) {
            return fig.getPreferredSize(askedBounds.width(), askedBounds.height()).getCopy();
        } else {
            return fig.getPreferredSize(askedBounds.width(), -1).getCopy();
        }
    }
