...
class NewIntersectionsRemover implements INewIntersectionsRemover {
    @objid ("a71fd36b-5c32-4fa2-9691-03625bda38c5")
    private boolean isDirty = true;

    /**
     * If true nodes will keep same distance.
    ...
    public void addBoundsChange(GraphicalEditPart node, Rectangle oldBounds, Rectangle newBounds) {
            ...
            for (Object o : clayer.getChildren()) {
                if (o instanceof Connection) {
                    ...
                    if (cp != null) {
                        ...
                        DiagramElements.LOG.warning("NewIntersectionsRemover: <%s> {%s} owned by <%s> has no edit part.", connection, connection.getPoints(), connection.getParent());
                    }
    ...
    private void cleanupConnections() {
        for (Iterator<Entry<ConnectionEditPart, PointList>> it = this.movableConnections.entrySet()
            ...
            if (isToExclude(cp)) {
                //DiagramElements.LOG.debug("  intersections: removing connection %s", dump(cp, new StringBuilder()));
                it.remove();
            }
        }
    }
    ...
    private void computeBoundsChange(GraphicalEditPart node, Rectangle oldBounds, Rectangle newBounds) {
        ...
        if (this.keepSameDistance) {
            ...
            for (Entry<GraphicalEditPart, Rectangle> entry : this.movableNodes.entrySet()) {
                ...
                if (childRect.intersects(oldBounds)) {
                    ...
                    if (isMove) {
                        pushDirection = computeMovePushDirection(scaleExpanded(newBounds, this.minDist), childRect);
                    } else {
                        pushDirection = computeResizePushDirection(oldBounds, childRect);
                    }
                    computePush(node, childRect, newBounds, pushDirection, neededPush);
                }
            }
                ...
                if (pts.intersects(newRect)) {
                    ...
                    for (int i=1, nb= pts.size(); i< nb-1; i++) {
                        ...
                        if (! oldBounds.contains(bendPointLoc)) {
                            ...
                            Dimension pushDirection;
                            if (isMove) {
                                pushDirection = computeMovePushDirection(scaleExpanded(newBounds, this.minDist), bendPointBounds);
                            } else {
    ...
    private Dimension computeMovePushDirection(Rectangle newBounds, Rectangle childRect) {
        if (newBounds.contains(childRect)) {
            ...
            computePushMoveFromIntersection(childRect, intersection, pushDirection);
    ...
    private void computePush(GraphicalEditPart cause, Rectangle toPush, Rectangle ref, Dimension oldVector, Move pushMove) {
        if (oldVector.width() > 0) {
            ...
            int dx = this.minDist + ref.right() - toPush.x();
            if (dx >0) {
            ...
            int dx = ref.x() - this.minDist - toPush.right();
            if (dx < 0) {
        ...
        if (oldVector.height() > 0) {
            ...
            int dy = this.minDist - toPush.y() + ref.bottom();
            if (dy > 0) {
            ...
            int dy = ref.y() - this.minDist - toPush.bottom();
            if (dy < 0) {
     ...
     * @param pushDirection the direction in which childRect must be pushed to avoid the intersection.
     */
    @objid ("be120aca-9f36-412c-b39b-8c970797ed55")
    private void computePushMoveFromIntersection(Rectangle childRect, Rectangle intersection, Dimension pushDirection) {
    ...
    private ChangeBoundsRequest createMoveRequest(GraphicalEditPart key, int dx, int dy) {
    ...
    private BpMove getBpMove(ConnectionEditPart c, int bendPointIndex, Point bpOrigLocation) {
        ...
        if (move == null) {
            move = new BpMove();
            move.origPoint = bpOrigLocation.getCopy();
    ...
    private static Rectangle scaleExpanded(Rectangle toExpand, int minValue) {
    ...
    private static class BoundChange {
        ...
        public BoundChange(GraphicalEditPart node, Rectangle oldBounds, Rectangle newBounds) {
            super();
            this.node = node;
    ...
    protected class ExecuteCommand extends CompoundCommand {
        ...
        public String getDebugLabel() {
            if (getCommands().isEmpty()) {
                ...
                return NewIntersectionsRemover.this.toString();
            }
        }
    ...
    private static class Move {
        ...
        public void setMaxLeft(int delta, int leftFrom, GraphicalEditPart leftCause) {
            int max = getMax(this.leftMove, delta);
            if (max == delta) {
                this.leftMove = max;
        ...
        public void setMaxRight(int delta, int rightFrom, GraphicalEditPart rightCause) {
            int max = getMax(this.rightMove, delta);
            if (max == delta) {
                this.rightMove = max;
        ...
        public void setMaxTop(int delta, int topFrom, GraphicalEditPart topCause) {
            int max = getMax(this.topMove, delta);
            if (max == delta) {
                this.topMove = max;
        ...
        public void setMaxBottom(int delta, int bottomFrom, GraphicalEditPart bottomCause) {
            int max = getMax(this.bottomMove, delta);
            if (max == delta) {
                this.bottomMove = max;
