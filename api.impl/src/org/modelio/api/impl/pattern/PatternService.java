...
public class PatternService implements IPatternService {
    ...
    public PatternService(IEclipseContext eclipseContext) {
    ...
    @Override
    public void exportPattern(Package modelPattern, Path patternPath) throws IOException {
    ...
    @Override
    public void addPattern(Path patternPath) throws IOException {
        try {
    ...
    @Override
    public Collection<String> getPatterns(Collection<MObject> elements) {
        Collection<String> ret = new ArrayList<>();
    ...
    @Override
    public boolean canApplyPattern(String patternName) throws IOException {
        RuntimePattern pattern = this.patternService.getCatalog().getPattern(patternName);
        if (pattern == null) {
            throw new InvalidParameterException("Invalid pattern " + patternName);
        }
    ...
    @Override
    public void applyPattern(String patternName, Map<String, Object> parameters) throws IOException {
        RuntimePattern pattern = this.patternService.getCatalog().getPattern(patternName);
        if (pattern == null) {
            throw new InvalidParameterException("Invalid pattern " + patternName);
        }
        pattern.applyPattern(parameters, projectService.getSession(), getModelRoot());
    }

    @objid ("ba0855f4-91e0-4d41-8cce-1383100d6265")
    @Override
    public void applyPattern(Path patternPath, Map<String, Object> parameters) throws IOException {
