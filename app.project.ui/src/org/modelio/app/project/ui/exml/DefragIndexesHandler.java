...
package org.modelio.app.project.ui.exml;
...
/**
 * Handler that defragment and compresses EXML indexes for the selected fragments.
 */
...
public class DefragIndexesHandler implements IRunnableWithProgress {
    ...
    void execute(@Named(IServiceConstants.ACTIVE_SELECTION) ITreeSelection sel, IModelioProgressService progressSvc, StatusReporter statusReporter) {
        for (Object o : sel.toList()) {
            ...
            String title = AppProjectUi.I18N.getMessage("DefragIndexesHandler.Title");
            try {
                progressSvc.run(title, true, false, this);
            } catch (InvocationTargetException e) {
                statusReporter.show(StatusReporter.ERROR, AppProjectUi.I18N.getMessage("RebuildIndexesHandler.Failed"), e.getCause());
            } catch (InterruptedException e) {
                statusReporter.show(StatusReporter.ERROR, AppProjectUi.I18N.getMessage("RebuildIndexesHandler.Failed"), e);
            }
        }
    }
