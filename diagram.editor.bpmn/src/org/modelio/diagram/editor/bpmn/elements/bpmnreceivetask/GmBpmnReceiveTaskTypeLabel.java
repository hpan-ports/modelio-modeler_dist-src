...
public class GmBpmnReceiveTaskTypeLabel extends GmDefaultModelElementHeader {
    ...
    public GmBpmnReceiveTaskTypeLabel(GmAbstractDiagram diagram, MRef relatedRef) {
    ...
    public void read(IDiagramReader in) {
        ...
        Object versionProperty = in.readProperty("GmBpmnTaskTypeLabel." + MINOR_VERSION_PROPERTY);
    ...
    public void write(IDiagramWriter out) {
        ...
        if (this.minorVersion != 0) {
            out.writeProperty("GmBpmnTaskTypeLabel." + MINOR_VERSION_PROPERTY, Integer.valueOf(this.minorVersion));
        }
    }
    ...
    protected String computeMainLabel() {
        Boolean showLabel = getStyle().getProperty(GmBpmnReceiveTaskStructuredStyleKeys.SHOWREPRESENTED);
        if (showLabel) {
            final BpmnReceiveTask task = (BpmnReceiveTask) getRelatedElement();
        
            if (task.isValid() && task.getCalledOperation() != null) {
                return task.getCalledOperation().getName() + "()";
            }
        }
        return "";
    }
