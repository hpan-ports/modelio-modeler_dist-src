...
public class RTPartitionScanner extends RuleBasedPartitionScanner {
    ...
    public RTPartitionScanner() {
        tagToken = new Token(RTPartitionTypes.OBJINGID_PARTITION);
        keywordToken = new Token(RTPartitionTypes.KEYWORD_PARTITION);
        commentToken = new Token(RTPartitionTypes.COMMENT_PARTITION);
                
        Vector<IPredicateRule> therules = new Vector<IPredicateRule>();
                
        KeywordRule keywordRule = new KeywordRule(keywordToken);
                
        for (int i = 0; i < keywords.length; i++)
            keywordRule.addKeyword(keywords[i], keywordToken);
                
        objidRule = new RTTagRule("@objid", tagToken, this);
                
        IPredicateRule comment = new CommentRule("//", commentToken, this);
        IPredicateRule multilineComment = new MultilineCommentRule("/*", "*/", commentToken, this);
        therules.add(keywordRule);
                
        therules.add(objidRule);
