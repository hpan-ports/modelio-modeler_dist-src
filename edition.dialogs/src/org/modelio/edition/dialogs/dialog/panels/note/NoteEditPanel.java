...
/*
    ...
    public void setInput(Object input) {
        // Bad input, clear all
        if (input == null || !(input instanceof Note)) {
            this.controller.setNote(null);
            return;
        }
        if (!((Note) input).isValid()) {
            this.controller.setNote(null);
            return;
        }
        
        // Input is a valid Note
        this.controller.setNote((Note) input);
    }
...
/*
    ...
    private static class NoteEditController {
        ...
        /**
         * Set the Note being edited. This method also refreshes the view contents.
         * @param note
         */
        ...
        private void setNoteMimeType(Note note, MimeType mimeType) {
            try (ITransaction t = CoreSession.getSession(note).getTransactionSupport().createTransaction("Change note mime type")) {
        ...
        private MimeType getNoteMimeMode(Note aNote) {
            if (aNote != null) {
                final String mimeType = aNote.getMimeType() != null && !aNote.getMimeType().isEmpty() ? aNote.getMimeType() : aNote
