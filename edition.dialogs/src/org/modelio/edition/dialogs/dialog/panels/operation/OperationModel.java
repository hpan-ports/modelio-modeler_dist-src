...
public class OperationModel implements IOperationPropertyModel {
    ...
    public void setReturnParameterType(GeneralClass type) {
        ...
        try (ITransaction t = session.getTransactionSupport().createTransaction("Set return parameter")) {
            if (type == null) {
                ...
                if (returnParameter == null) {
                    ModelFactory.getFactory(session).createReturnParameter("", type, this.operation);
                } else {
    ...
    public void addParameter() {
        ...
        try (ITransaction t = session.getTransactionSupport().createTransaction("Add parameter")) {
            final Parameter parameter = ModelFactory.getFactory(session).createParameter();
    ...
    private boolean moveOneParameterDown(Parameter p) {
        ...
        // Important note:
        // Collections.swap(parameters, pos, pos - 1) CANNOT be used directly on the list returned by this.operation.getIO()
        // as it is a SmList whose redefinition of List<?>#set() use semantic accessors like setDepVal
