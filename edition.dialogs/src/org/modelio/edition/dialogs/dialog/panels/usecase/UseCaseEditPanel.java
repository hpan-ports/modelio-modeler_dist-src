...
public class UseCaseEditPanel implements IPanelProvider {
    @objid ("46f8521f-0b35-4cda-af4d-ed4fef580a8a")
    private static final String HELP_TOPIC = EditionDialogs.I18N.getString("EditElementDialog.UseCase.HELP_TOPIC");
    ...
    @Override
    public void setInput(Object editedUseCase) {
        // Bad input, clear all
        if (editedUseCase == null || !(editedUseCase instanceof UseCase)) {
            this.uc = null;
            return;
        }
        if (!((UseCase) editedUseCase).isValid()) {
            this.uc = null;
            return;
        }
        
        // Input is a valid UseCase
        // Create fields only if first time
        final boolean firstTime = this.uc == null;
        this.uc = (UseCase) editedUseCase;
    ...
    void createFormFields(Composite comp) {
        ...
        this.description = new NoteField(this.toolkit, folder, new NoteFieldData(this.uc, "ModelerModule", "description"));
